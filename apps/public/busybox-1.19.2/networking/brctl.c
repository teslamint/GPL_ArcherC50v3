...
int brctl_main(int argc UNUSED_PARAM, char **argv)
{
	...
	while (*argv) {
		...
		if (key == ARG_setifvlan) {
			...
			vlan = xatoi_positive(*argv++);
			arm_ioctl(args, BRCTL_SET_IF_VLAN, if_nametoindex(brif), vlan);
			xioctl(fd, SIOCDEVPRIVATE, &ifr);
			goto done;
		}
		...
		if (key == ARG_showvlan) {
			strncpy_IFNAMSIZ(ifr.ifr_name, br);
			arm_ioctl(args, BRCTL_SHOW_VLAN, 0, 0);
			xioctl(fd, SIOCDEVPRIVATE, &ifr);
			goto done;
		}
		...
		if (key == ARG_setpathcost
		 ...
		 || key == ARG_setbrsnooping
			...
			if ((key != ARG_setbridgeprio) && (key != ARG_setbrsnooping)){
				/* get portnum */
				unsigned i;
