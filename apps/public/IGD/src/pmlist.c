...
int pmlist_AddPortMapping_ForwardList(int enabled, char *protocol, char *externalPort, char *internalClient, char *internalPort)
{
	...
	if (enabled && g_vars.forwardRules)
	{
		...
		trace(3, "%s", command);
		pmipt_System(command);
#ifdef INCLUDE_MEXICO_SPEC
		sprintf(command, "echo \"-a2 %s %s %s %s %s\" > /proc/tplink/echan",
				internalClient,
				internalPort,
				externalPort,
				externalPort,
...
int pmlist_AddPortMapping (int enabled, char *protocol, char *externalPort, char *internalClient, char *internalPort)
{
	 ...
	 if(strncmp(g_vars.extInterfaceName, g_vars.primInterfaceName,IFNAMSIZ))
	  {
	  	  ...
	  	  if (0 == getIpAddress(g_vars.primInterfaceName, wanIpStr,  IPSTR_LEN))
		  {
			if(strcmp(wanIpStr, g_vars.primIntfAddr))
			{
		  	...
		  	{
		  		memset(g_vars.primIntfAddr, 0, 16);
			  	{
					  snprintf(command, COMMAND_LEN, "%s -t nat -F %s",
						  g_vars.iptables, "PREROUTING_UPNP_SECCONN"/*g_vars.preroutingChainName*/);
					  trace(3, "%s", command);
	 ...
	 */
	  ...
	  {
		snprintf(command, COMMAND_LEN, "%s -t nat -A %s -m conntrack --ctorigdst %s -o %s -d %s -p %s --dport %s -j SNAT --to-source %s", g_vars.iptables, "POSTROUTING_NATLOOPBACK_UPNP", wanIpStr,
	  ...
	  if(strncmp(g_vars.extInterfaceName, g_vars.primInterfaceName,IFNAMSIZ))
	  {
	  	if (0 == getIpAddress(g_vars.primInterfaceName, wanIpStr, IPSTR_LEN))
	  	{
	  		if(!IS_PRV_IP(wanIpStr))
			{
				if( !getLanInterface(lanIfName, internalClient, IFNAMSIZ))
				{
					snprintf(command, COMMAND_LEN, "%s -t nat -A %s -m conntrack --ctorigdst %s -o %s -d %s -p %s --dport %s -j SNAT --to-source %s", g_vars.iptables, "NATLOOPBACK_UPNP_SECCONN", wanIpStr,
					lanIfName, internalClient, protocol, internalPort, wanIpStr);
					trace(3, "%s", command);
					pmipt_System(command);
				}
...
int pmlist_DeletePortMapping_ForwardList(int enabled, char *protocol, char *externalPort, char *internalClient, char *internalPort)
{
	...
	if (enabled && g_vars.forwardRules)
	{
		...
		pmipt_System(command);
#ifdef INCLUDE_MEXICO_SPEC
		sprintf(command, "echo \"-d2 %s %s %s %s %s\" > /proc/tplink/echan",
				internalClient,
				internalPort,
				externalPort,
				externalPort,
...
int pmlist_DeletePortMapping(int enabled, char *protocol, char *externalPort, char *internalClient, char *internalPort)
{
	...
	#endif
	{
	 ...
	 /* 
	  ...
	  {
		snprintf(command, COMMAND_LEN, "%s -t nat -D %s -m conntrack --ctorigdst %s -o %s -d %s -p %s --dport %s -j SNAT --to-source %s", g_vars.iptables, "POSTROUTING_NATLOOPBACK_UPNP", wanIpStr,
		lanIfName, internalClient, protocol, internalPort, wanIpStr);
		trace(3, "%s", command);
		pmipt_System(command);
	  }
	  ...
	  if(strncmp(g_vars.extInterfaceName, g_vars.primInterfaceName,IFNAMSIZ))
	  {
	  	  	...
	  	  	if(!IS_PRV_IP(wanIpStr))
			{
			  ...
			  pmipt_System(command);
			   ...
			   {
			   		snprintf(command, COMMAND_LEN, "%s -t nat -D %s -m conntrack --ctorigdst %s -o %s -d %s -p %s --dport %s -j SNAT --to-source %s", g_vars.iptables, "NATLOOPBACK_UPNP_SECCONN", wanIpStr,
					lanIfName, internalClient, protocol, internalPort, wanIpStr);
					trace(3, "%s", command);
					pmipt_System(command);
			   }
