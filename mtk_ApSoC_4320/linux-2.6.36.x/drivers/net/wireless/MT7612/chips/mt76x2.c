...
static void mt76x2_switch_channel(RTMP_ADAPTER *ad, u8 channel, BOOLEAN scan)
{
	...
	/* enable TX/RX */
	RTMP_IO_WRITE32(ad, MAC_SYS_CTRL, 0xc);
...
void mt76x2_get_external_lna_gain(RTMP_ADAPTER *ad)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s::LNA type=0x%x, BLNAGain=0x%x, ALNAGain0=0x%x, ALNAGain1=0x%x, ALNAGain2=0x%x\n",  
...
void mt76x2_get_agc_gain(RTMP_ADAPTER *ad, BOOLEAN init_phase)
{
	...
	if (init_phase == TRUE) {
		...
		DBGPRINT(RT_DEBUG_TRACE, ("original vga value(chain0) = %x\n",  ad->CommonCfg.lna_vga_ctl.agc_vga_ori_0));
	}
	...
	if (ad->CommonCfg.RxStream >= 2) {
		...
		if (init_phase == TRUE) {
			...
			DBGPRINT(RT_DEBUG_TRACE, ("original vga value(chain1) = %x\n",  ad->CommonCfg.lna_vga_ctl.agc_vga_ori_1));
		}
		val16 = ((bbp_val & (0xffff0000)) >> 16) & (0xffff);
...
static u8 mt76x2_txpwr_chlist[] = {
	...
	36,38,40,44,46,48,52,54,56,60,62,64,			// 12
	100,102,104,108,110,112,116,118,120,124,126,128,132,134,136,140,144,	// 17
	149,151,153,157,159,161,165,167,169,171,173,	// 11
	42, 58, 106, 122, 155,							// 5
};
...
void percentage_delta_pwr(RTMP_ADAPTER *ad) 
{
	...
	*/
	...
	tx_alc_ch_init_0 = (mac_val & TX_ALC_CFG_0_CH_INT_0_MASK) + mac_drop_pwr*2;
	if (tx_alc_ch_init_0 <= 0)
		tx_alc_ch_init_0 = 0;
	tx_alc_ch_init_1 = ((mac_val & TX_ALC_CFG_0_CH_INT_1_MASK) >> 8) + mac_drop_pwr*2;
...
void mt76x2_read_temp_info_from_eeprom(RTMP_ADAPTER *ad)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s:: is_temp_tx_alc=%d, temp_tx_alc_enable=%d\n", 
...
void mt76x2_read_tx_alc_info_from_eeprom(RTMP_ADAPTER *ad)
{
	...
	*/
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s:: is_ePA_mode=%d, ePA_type=%d\n", 
		__FUNCTION__, is_ePA_mode, ePA_type));
	DBGPRINT(RT_DEBUG_TRACE, ("%s:: [5G band] high_temp_slope=%d, low_temp_slope=%d\n", 
		__FUNCTION__, pChipCap->high_temp_slope_a_band, pChipCap->low_temp_slope_a_band));
	DBGPRINT(RT_DEBUG_TRACE, ("%s:: [2G band] high_temp_slope=%d, low_temp_slope=%d\n", 
		__FUNCTION__, pChipCap->high_temp_slope_g_band, pChipCap->low_temp_slope_g_band));
	DBGPRINT(RT_DEBUG_TRACE, ("%s:: [5G band] tc_lower_bound=%d, tc_upper_bound=%d\n", 
		__FUNCTION__, pChipCap->tc_lower_bound_a_band, pChipCap->tc_upper_bound_a_band));
	DBGPRINT(RT_DEBUG_TRACE, ("%s:: [2G band] tc_lower_bound=%d, tc_upper_bound=%d\n", 
...
void mt76x2_single_sku(RTMP_ADAPTER *ad, u8 channel)
{
	...
	if ((ad->chipCap.temp_tx_alc_enable == TRUE) && (ad->chipCap.tssi_enable == FALSE)) {
		...
		DBGPRINT(RT_DEBUG_OFF, ("%s::sku_base_pwr = 0x%x, DefaultTargetPwr = 0x%x, ch_pwr_adj = 0x%x(%d), 0x13B4: 0x%x\n", 
...
INT32 mt76x2_sku_calculate_TxPwrAdj(RTMP_ADAPTER *ad, struct _TXWI_NMAC *txwi_n)
{
	...
	if (txwi_n->PHYMODE == MODE_HTMIX || txwi_n->PHYMODE == MODE_HTGREENFIELD || txwi_n->PHYMODE == MODE_VHT)
	{
		...
		{
			INT32 sku_pwr_HT20 = 0;	//ad->chipCap.rate_pwr_table.HT20[txwi_n->MCS].sku_pwr;
			INT32 sku_pwr_HT40 = 0;	//ad->chipCap.rate_pwr_table.HT40[txwi_n->MCS].sku_pwr;
			...
			if (txwi_n->MCS <= MCS_7)
			{
				sku_pwr_HT20 = ad->chipCap.rate_pwr_table.HT20[txwi_n->MCS].sku_pwr;
				sku_pwr_HT40 = ad->chipCap.rate_pwr_table.HT40[txwi_n->MCS].sku_pwr;		
			}
			else if (txwi_n->PHYMODE == MODE_VHT)
			{
				/* VHT 1ss/2ss MCS8/MCS9 will not be adjusted */
				/* 16 (0x10) is offset of VHT 2ss MCS index, mismatch with defination in rtmp_def.h */
				if ((0x10 <= txwi_n->MCS) && (txwi_n->MCS <= 0x17))		/* 11acVHT 2ss */
				{
					sku_pwr_HT20 = ad->chipCap.rate_pwr_table.HT20[txwi_n->MCS-0x10].sku_pwr;
					sku_pwr_HT40 = ad->chipCap.rate_pwr_table.HT40[txwi_n->MCS-0x10].sku_pwr;
				}
			}
			else
			{
				if ((MCS_8 <= txwi_n->MCS) && (txwi_n->MCS <= MCS_15))	/* 11nHT 2ss */
				{
					sku_pwr_HT20 = ad->chipCap.rate_pwr_table.HT20[txwi_n->MCS].sku_pwr;
					sku_pwr_HT40 = ad->chipCap.rate_pwr_table.HT40[txwi_n->MCS].sku_pwr;
				}
		...
		{
			/* avoid array out of index access !!! */
			INT32 sku_pwr_HT20 = 0;	//ad->chipCap.rate_pwr_table.HT20[txwi_n->MCS].sku_pwr;
			INT32 sku_pwr_HT40 = 0;	//ad->chipCap.rate_pwr_table.HT40[txwi_n->MCS].sku_pwr;
			...
			else if (txwi_n->PHYMODE == MODE_VHT)
			{
				...
				if ((0x10 <= txwi_n->MCS) && (txwi_n->MCS <= 0x17))		/* 11acVHT 2ss */
				{
					...
					if (txwi_n->BW == BW_40)
						sku_table_delta = sku_pwr_HT40 - ad->chipCap.rate_pwr_table.VHT2SS[txwi_n->MCS-0x10].sku_pwr;
					else if (txwi_n->BW == BW_20)
						sku_table_delta = sku_pwr_HT20 - ad->chipCap.rate_pwr_table.VHT2SS[txwi_n->MCS-0x10].sku_pwr;
				}
			}
			else
			{
				if ((MCS_8 <= txwi_n->MCS) && (txwi_n->MCS <= MCS_15))	/* 11nHT 2ss */
				{
					...
					if (txwi_n->BW == BW_40)
						sku_table_delta = sku_pwr_HT40 - ad->chipCap.rate_pwr_table.VHT2SS[txwi_n->MCS-MCS_8].sku_pwr;
					else if (txwi_n->BW == BW_20)
						sku_table_delta = sku_pwr_HT20 - ad->chipCap.rate_pwr_table.VHT2SS[txwi_n->MCS-MCS_8].sku_pwr;
				}
			}

			if (ad->bSingleSkuDebug)
			{
				DBGPRINT(RT_DEBUG_OFF, ("%s HT%s[%d] SKU PWR: %d VHT[%d] SKU PWR: %d\n", 
...
void MT76x2_AsicDynamicVgaGainControl(RTMP_ADAPTER *pAd)
{
	if ((pAd->CommonCfg.lna_vga_ctl.bDyncVgaEnable) &&
		...
		DBGPRINT(RT_DEBUG_INFO, ("MT76x2 one second False CCA=%d, fixed agc_vga_0:0%x, fixed agc_vga_1:0%x\n", pAd->RalinkCounters.OneSecFalseCCACnt, val1, val2));
...
VOID WlanResetB(RTMP_ADAPTER *pAd)
{
	...
	DBGPRINT(RT_DEBUG_TRACE, ("%s: MAC_SYS_CTRL =%08x \n", __FUNCTION__, MacValue));	
