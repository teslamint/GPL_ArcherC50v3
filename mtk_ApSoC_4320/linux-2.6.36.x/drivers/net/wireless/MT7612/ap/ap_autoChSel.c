	...
	)
{
	...
	for (channel_idx = 0; channel_idx < pAd->ChannelListNum; channel_idx++)
	{
		...
		if (pChannelInfo->FalseCCA[channel_idx] > CCA_THRESHOLD)
		{
			...
			/* when busy time of a channel > threshold, skip it */
			/* TODO: Use weight for different references to do channel selection */
			if (QBSS_LoadIsBusyTimeAccepted(pAd, pChannelInfo->chanbusytime[channel_idx]) == FALSE)
			{
				/* check next one */
				continue;
			}
	...
	/* For FCC Country, if there is no channel selected by the acs process, set ch 153 as default. */
	if ((channel_idx == pAd->ChannelListNum) && (pAd->CommonCfg.CountryRegionForABand == 0))
	{
		ch = 153;
		DBGPRINT(RT_DEBUG_WARN, ("(Region: %d) No candidate for Auto Channel Select ==> default as: %d\n", 
			pAd->CommonCfg.CountryRegionForABand, ch));
	}
