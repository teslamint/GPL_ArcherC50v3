...
COUNTRY_CODE_TO_COUNTRY_REGION allCountry[] = {
	...
	{76,	"BR",	"BRAZIL",				TRUE,	A_BAND_REGION_0,	TRUE,	G_BAND_REGION_0},
	...
	{410,	"KR",	"KOREA REPUBLIC OF",	TRUE,	A_BAND_REGION_0,	TRUE,	G_BAND_REGION_1},
	...
	{642,	"RO",	"ROMANIA",				FALSE,	A_BAND_REGION_1,	TRUE,	G_BAND_REGION_1},
	{643,	"RU",	"RUSSIA FEDERATION",	FALSE,	A_BAND_REGION_9,	TRUE,	G_BAND_REGION_1},
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
	...
	/* yanglx: port from MT7610, Change ',' to '\b', 
	 * NO support of setting multi cmds at one time, such as "iwpriv ra0 set SSID=ABC,Channel=1" */
	//while ((this_char = strsep((char **)&pIoctlCmdStr->u.data.pointer, ",")) != NULL) 
	while ((this_char = strsep((char **)&pIoctlCmdStr->u.data.pointer, "\b")) != NULL) 
	{
		if (!*this_char)
			 continue;
...
/*	msg = (PSTRING)kmalloc(sizeof(CHAR)*(2048), MEM_ALLOC_FLAG); */
	...
	//		sprintf(msg+strlen(msg), "WPS Query Status(ra%d)          = %d\n", idx, pAd->ApCfg.MBSSID[idx].WscControl.WscStatus);
			sprintf(msg+strlen(msg), "WPS Wsc2MinsTimerRunning(%s%d)        = %d\n", INF_MAIN_DEV_NAME, idx, pAd->ApCfg.MBSSID[idx].WscControl.Wsc2MinsTimerRunning);
		}
#else
		for (idx = 0; idx < pAd->ApCfg.BssidNum; idx++)
		{
			...
			if (pAd->ApCfg.MBSSID[idx].WscControl.WscEnrolleePinCodeLen == 8)
				sprintf(msg+strlen(msg), "Enrollee PinCode(%s%d)           %08u\n", INF_MAIN_DEV_NAME, idx, pAd->ApCfg.MBSSID[idx].WscControl.WscEnrolleePinCode);
			else
				sprintf(msg+strlen(msg), "Enrollee PinCode(%s%d)           %04u\n", INF_MAIN_DEV_NAME, idx, pAd->ApCfg.MBSSID[idx].WscControl.WscEnrolleePinCode);
		    sprintf(msg+strlen(msg), "WPS Profile Count               = %d\n", pAd->ApCfg.MBSSID[idx].WscControl.WscProfile.ProfileCnt);
		}
#endif
	}
	...
	{
	if (pAd->ApCfg.ApCliTab[i].WscControl.WscEnrolleePinCodeLen == 8)
		sprintf(msg+strlen(msg), "Enrollee PinCode(%s%d)        %08u\n", INF_APCLI_DEV_NAME, i, pAd->ApCfg.ApCliTab[i].WscControl.WscEnrolleePinCode);
	else
		sprintf(msg+strlen(msg), "Enrollee PinCode(%s%d)        %04u\n", INF_APCLI_DEV_NAME, i, pAd->ApCfg.ApCliTab[i].WscControl.WscEnrolleePinCode);
    sprintf(msg+strlen(msg), "Ap Client WPS Profile Count     = %d\n", pAd->ApCfg.ApCliTab[i].WscControl.WscProfile.ProfileCnt);
    for (idx = 0; idx < pAd->ApCfg.ApCliTab[i].WscControl.WscProfile.ProfileCnt ; idx++)
    {
        PWSC_CREDENTIAL pCredential = &pAd->ApCfg.ApCliTab[i].WscControl.WscProfile.Profile[idx];
...
Arguments:
    ...
    Note:	Writen by Zeng Yi. 2011-10-22.
    		Used to get Bss Information.
    ==========================================================================
*/
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	bss.rssi = (unsigned int)(pAd->ate.LastRssi0 - pAd->BbpRssiToDbmDelta);
	bss.channel = (unsigned int)pAd->CommonCfg.Channel; 
    wrq->u.data.length = sizeof(bss);
    copy_to_user(wrq->u.data.pointer, &bss, wrq->u.data.length);
}

/* 
    ==========================================================================
    Description:
        Get ap rate information
Arguments:
    pAdapter                    Pointer to our adapter
    wrq                         Pointer to the ioctl argument
    ...
    Note:	Writen by Zeng Yi. 2011-10-22.
    		Revised by Wangbing.2013-11-04.
    		Used to get BssRate Information.
    ==========================================================================
*/
...
{
	WLAN_RATE_INFO rate;
	POS_COOKIE	pObj = pAd->OS_Cookie;
	INT rateMask[] = { 12, 18, 24, 36, 48, 72, 96, 108 };
	INT rateMaskLen = sizeof(rateMask) / sizeof(INT);
	INT apidx = pObj->ioctl_if;
	INT mcs = pAd->ApCfg.MBSSID[apidx].wdev.DesiredTransmitSetting.field.MCS;
	INT i, j = 0;
	UCHAR phyMode = pAd->CommonCfg.PhyMode;
	memset(&rate, 0x00, sizeof(rate));

	rate.basicRateMask = pAd->CommonCfg.BasicRateBitmap;
	rate.basicMcsMask = 0;
	
	for (i = 0; i < rateMaskLen; i++)
	{
		if (pAd->CommonCfg.DesireRate[i] == rateMask[i])
		{
			rate.possibleRateMask |= 1 << i;
			rate.operationalRateMask |= 1 << i;
		}
	}
	...
	case PHY_11AN_MIXED:
		if (MCS_AUTO == mcs)
		{
			if (1 == pAd->CommonCfg.TxStream)
			{
				for (i = 0; i <= 7; i++)
				{
					rate.operationalMcsMask |= 1 << i;
					rate.possibleMcsMask |= 1 << i;
				}
			}
			else if (2 == pAd->CommonCfg.TxStream)
			{
				for (i = 0; i <= 15; i++)
				{
					rate.operationalMcsMask |= 1 << i;
		...
		{
			rate.operationalMcsMask |= 1 << (mcs - 1);
			rate.possibleMcsMask |= 1 << (mcs - 1);
		}
		break;
	...
	case PHY_11VHT_N_MIXED:
		...
		break;
	default:
		rate.operationalMcsMask = 0;
		rate.possibleMcsMask = 0;
		break;
	}
	wrq->u.data.length = sizeof(rate);
	copy_to_user(wrq->u.data.pointer, &rate, wrq->u.data.length);
}
...
{
	int index;
	WLAN_STA_INFO *staInfo;	
	unsigned int maxStaNum;
	unsigned int assocNum = 0;
	int len = 0;
	unsigned char *c = (char*)(&maxStaNum);
	POS_COOKIE	pObj = pAd->OS_Cookie;
	INT apidx = pObj->ioctl_if;
	unsigned char vapName[16] = {0};

	len = copy_from_user(&maxStaNum, wrq->u.data.pointer, sizeof(unsigned int));  
	if (len != 0)
	{
	...
	staInfo = (WLAN_STA_INFO *)kmalloc(sizeof(WLAN_STA_INFO)*maxStaNum, MEM_ALLOC_FLAG);
	if (NULL == staInfo)
	{
		printk("%s, Failed to kmalloc\n", __FUNCTION__);
		return;
	}
	memset(staInfo, 0x00, sizeof(WLAN_STA_INFO)*maxStaNum);
	sprintf(vapName, "rai%d", apidx);
	
	for (index = 0; index < maxStaNum; index++)
	{
		PMAC_TABLE_ENTRY pEntry = &pAd->MacTab.Content[index];
		if ((IS_ENTRY_CLIENT(pEntry) || IS_ENTRY_APCLI(pEntry))
			&& (pEntry->Sst == SST_ASSOC) && (pAd->MacTab.Content[index].apidx == apidx))
		{
			memcpy(staInfo[assocNum].addr, pEntry->Addr, MAC_ADDR_LEN);
			staInfo[assocNum].rx_packets = (UINT32)pEntry->RxPackets.QuadPart;
			staInfo[assocNum].tx_packets = (UINT32)pEntry->TxPackets.QuadPart;
			memcpy(staInfo[assocNum].hostName, vapName, 16);
			assocNum++;
		}
	}

	copy_to_user(wrq->u.data.pointer, &assocNum, sizeof(unsigned int));
	copy_to_user(wrq->u.data.pointer + sizeof(unsigned int), staInfo, sizeof(WLAN_STA_INFO) * assocNum);
	kfree(staInfo);
}

/* we should better skip before scan??? */
/* other skip logic may be added, 
 * but now only dfs channels skip is handled which depends on whether DFS_SUPPORT is configured or not. */
/* AutoChannelSkipList is not suitable here, 
 * e.g. if we have dfs on, 
 * we may still do not want auto channel in band2/band3, 
 * but band2/band3 should be valid for wds scan. */
static int scan_need_skip_bss(PRTMP_ADAPTER pAd, BSS_ENTRY *pBss)
{
	int i;
	...
	{
		//if (channel == pAd->ApCfg.AutoChannelSkipList[i])
		if ((pBss->Channel == pAd->ChannelList[i].Channel) && pAd->ChannelList[i].DfsReq
#ifdef DFS_SUPPORT
			&& FALSE	/* hard code here */
#endif
			)
		{
			return TRUE;
...
Arguments:
    ...
    Note:	Writen by Zeng Yi. 2011-10-22.
    		Used to get ScanBss Information.
    ==========================================================================
*/
...
#define RSSI_BASE (-96)
	...
	UINT32		WaitCnt;
	BSS_ENTRY	*pBss;
	WLAN_BSS_INFO *pBssInfo;
	int length = 0;
	UINT32 maxBssNum;
	UINT32 validBssNum;
	UINT32 rssi_dbm;

	length = copy_from_user(&maxBssNum, wrq->u.data.pointer, sizeof(unsigned int));
	if (length != 0)
	{
		printk("Error Read from user\n");
		return;
	}

	if (maxBssNum > (MAX_LEN_OF_BSS_TABLE))
	{
		maxBssNum = (MAX_LEN_OF_BSS_TABLE);
	}

	DBGPRINT(RT_DEBUG_TRACE, ("Max Bss Table is %d, MAX_LEN is : %d\n", maxBssNum, MAX_LEN_OF_BSS_TABLE));

	pBssInfo = (WLAN_BSS_INFO *)kmalloc((maxBssNum * sizeof(WLAN_BSS_INFO)), MEM_ALLOC_FLAG);
	if (NULL == pBssInfo)
	{
		printk("%s, Failed to kmalloc\n", __FUNCTION__);
		return;
	...
	do		/* yanglx: mt7612e needs wait for a while before checking scan status */
	{
		OS_WAIT(500);	
	}
	while ((ScanRunning(pAd) == TRUE) && (WaitCnt++ < 200))/*OS_WAIT(500)*/;

	DBGPRINT(RT_DEBUG_TRACE, ("pAd->ScanTab.BssNr is %d\n", pAd->ScanTab.BssNr));
	
	for (i = 0, validBssNum = 0; (i < pAd->ScanTab.BssNr) && (validBssNum < maxBssNum); i++)
	{
		...
		if (scan_need_skip_bss(pAd, pBss))
			continue;

		pBssInfo[validBssNum].channel = (unsigned int)pBss->Channel;
		rssi_dbm = (UINT32)(pBss->Rssi - RSSI_BASE);
		pBssInfo[validBssNum].rssi = rssi_dbm;
		if (FALSE == pBss->Privacy)
		{
			pBssInfo[validBssNum].securityEnable = FALSE;
		}
		else
		{
			pBssInfo[validBssNum].securityEnable = TRUE;
		}
		pBssInfo[validBssNum].authMode = pBss->AuthMode;
		pBssInfo[validBssNum].wepStatus = pBss->WepStatus;

		memcpy(pBssInfo[validBssNum].ssid, pBss->Ssid, MAX_LEN_OF_SSID);
		memcpy(pBssInfo[validBssNum].bssid, pBss->Bssid, MAC_ADDR_LEN);

		DBGPRINT(RT_DEBUG_TRACE, ("Bss %s: auth = %d, wepStatus = %d, rssi = %d(%d)\n", 
			pBss->Ssid, pBss->AuthMode, pBss->WepStatus, pBss->Rssi, pBssInfo[validBssNum].rssi));

		++validBssNum;
	}

	wrq->u.data.length = sizeof(CHAR)*(validBssNum * sizeof(WLAN_BSS_INFO)) + sizeof(unsigned int);
	copy_to_user(wrq->u.data.pointer, &validBssNum, sizeof(validBssNum));
	copy_to_user(wrq->u.data.pointer + sizeof(validBssNum), pBssInfo, (validBssNum * sizeof(WLAN_BSS_INFO)));
	kfree(pBssInfo);
}
	...
	IN	PSTRING			arg)
{
...
#endif /* APCLI_SUPPORT */
    {
        bFromApCli = FALSE;
        pWscControl = &pAd->ApCfg.MBSSID[apidx].WscControl;
        DBGPRINT(RT_DEBUG_TRACE, ("IF(%s%d) Set_AP_WscGetConf_Proc:: This command is from ra interface now.\n", 
			INF_MAIN_DEV_NAME, apidx));
    }
	...
	IN	ULONG					Data)
{
	...
	switch(cmd)
	{
		...
		/* yanglx: prot from MT7610 */
		case CMD_RTPRIV_IOCTL_GBSSINFO:
			RTMPIoctlGBSSINFO(pAd, wrq);
			break;
		case CMD_RTPRIV_IOCTL_GRATEINFO:
			RTMPIoctlGRATEINFO(pAd, wrq);
			break;
		case CMD_RTPRIV_IOCTL_GSTAINFO:
			RTMPIoctlGSTAINFO(pAd, wrq);
			break;
		case CMD_RTPRIV_IOCTL_GSCANINFO:
			RTMPIoctlGSCANBSSINFO(pAd, wrq);
			break;
		/* yanglx ended */
