...
#if 0		/* yanglx, 2015-11-6: remove Country Information from Beacon frame */
	...
	*/
	...
	else if (NumOfMacs <= 4)
	{
		if (pAd->CurrentAddress[5] % 4 != 0)
			DBGPRINT(RT_DEBUG_ERROR, ("%s: (%d) The 4-BSSID mode is enabled, the BSSID byte5 MUST be the multiple of 4\n", __FUNCTION__, NumOfMacs));
