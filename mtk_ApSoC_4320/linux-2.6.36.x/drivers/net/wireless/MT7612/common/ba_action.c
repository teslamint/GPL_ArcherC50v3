...
/*	if ((RTMP_TIME_AFTER((unsigned long)Now32, (unsigned long)(pBAEntry->LastIndSeqAtTimer+REORDERING_PACKET_TIMEOUT)) &&
	   ...
	   )
	{
		...
		DBGPRINT(RT_DEBUG_TRACE, ("timeout[%d] (%lx-%lx = %d > %d): %x, flush one!", 
			pBAEntry->list.qlen, Now32, (pBAEntry->LastIndSeqAtTimer),
			(int)((long) Now32 - (long)(pBAEntry->LastIndSeqAtTimer)), REORDERING_PACKET_TIMEOUT,
			pBAEntry->LastIndSeq));
	}

	/* what about the others? not handle??? */
}
...
BA_REC_ENTRY *BATableAllocRecEntry(RTMP_ADAPTER *pAd, USHORT *Idx)
{
	...
	{
		DBGPRINT(RT_DEBUG_ERROR, ("BA Recipient Session (%ld) > %d\n", 
...
BA_ORI_ENTRY *BATableAllocOriEntry(RTMP_ADAPTER *pAd, USHORT *Idx)
{
	...
	{
		DBGPRINT(RT_DEBUG_ERROR, ("BA Originator Session (%ld) > %d\n", 
							pAd->BATable.numAsOriginator, (MAX_LEN_OF_BA_ORI_TABLE - 1)));
		goto done;
	}
	...
	if (pBAEntry->ORI_BA_Status != Originator_NONE)
	{
		...
		if (pBAEntry->ORI_BA_Status == Originator_Done)
		{
			pAd->BATable.numDoneOriginator -= 1;
		 	pEntry->TXBAbitmap &= (~(1 << (pBAEntry->TID)));	/* Erase Bitmap flag.*/
		}
