...
out:
	DBGPRINT(RT_DEBUG_TRACE, ("%s::e2p_dafault = %d\n", __FUNCTION__, e2p_dafault));
	return e2p_dafault;
}
...
static VOID RtmpEepromTypeAdjust(RTMP_ADAPTER *pAd, UCHAR *pE2pType)
{
	...
	{
		DBGPRINT(RT_DEBUG_OFF, ("%s: NVM is efuse and the information is too less to bring up the interface\n", __FUNCTION__));
		DBGPRINT(RT_DEBUG_OFF, ("%s: Force to use Flash mode\n", __FUNCTION__));
