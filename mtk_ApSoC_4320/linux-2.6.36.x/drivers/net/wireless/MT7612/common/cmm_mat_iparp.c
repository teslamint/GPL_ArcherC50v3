...
/*dumpPkt(RTPKT_TO_OSPKT(pSkb)->data, RTPKT_TO_OSPKT(pSkb)->len); */
	...
	if (isUcastMac && isGoodIP)
	{
		...
		/* if can't find the ip-mac rule and the dstip is in different ip section with srcip, 
		 * don't change the mac */
		if (((pRealMac[0] & pRealMac[1] & pRealMac[2] & pRealMac[3] & pRealMac[4] & pRealMac[5]) == 0xff)
			&& ((dstIP & 0x00FFFFFF) != (srcIP & 0x00FFFFFF)))	/* hard coded mask??? */
		{
				pRealMac = tgtMac;
		}
	}
