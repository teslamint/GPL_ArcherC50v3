	...
	IN PSTRING buffer)
{
	...
	if (RTMPGetKeyParameter("ApCliWPAPSK", tmpbuf, 255, buffer, FALSE))
	{
		for (i = 0, macptr = rstrtok(tmpbuf,";"); (macptr && i < MAX_APCLI_NUM); macptr = rstrtok(NULL,";"), i++)
		{
			...
			for (j = 0; j < 32; j++)
			{
				DBGPRINT(RT_DEBUG_TRACE, ("%02x:", pApCliEntry->PMK[j]));
				if ((j%16) == 15)
...
void RTMPSetCountryCode(RTMP_ADAPTER *pAd, PSTRING CountryCode)
{
	...
	if (strlen((PSTRING) pAd->CommonCfg.CountryCode) != 0)
		pAd->CommonCfg.bCountryFlag = FALSE;	// TRUE;	/* always hide country code */
	else
...
/*#endif*/
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			...
			if (RTMPGetKeyParameter("AutoChannelSkipList", tmpbuf, 128, pBuffer, FALSE))
			{
				...
				{
					DBGPRINT(RT_DEBUG_WARN, ("Your no. of AutoChannelSkipList( %d ) is larger than %d (boundary)\n",
