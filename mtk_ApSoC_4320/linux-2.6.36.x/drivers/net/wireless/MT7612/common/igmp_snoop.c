	...
	IN MulticastFilterEntryType type)
{
	...
	if ((pEntry = MulticastFilterTableLookup(pMulticastFilterTable, pGrpId, dev)))
	{
		...
		while (pMemberEntry)
		{
			...
			{
				DBGPRINT(RT_DEBUG_WARN, ("%s: already in Members list.\n", __FUNCTION__));
				return FALSE;
			}
	...
	IN PNET_DEV dev)
{
	...
	do
	{
		...
		{
			DBGPRINT(RT_DEBUG_WARN, ("%s: the Group doesn't exist.\n", __FUNCTION__));
		}
	} while(FALSE);
...
static int isDeniedGroup(PUCHAR pGroupIpAddr)
{
    UINT32  denyList[] = {
        ...
        0xEFFFFFFA,  /* 239.255.255.250, UPNP/SSDP */
...
/* deprecated, replaced by isDeniedGroup */
/* yanglx, 2014-9-18: ported from MT7610E */
BOOLEAN isInvalidM2UIP(
	IN PRTMP_ADAPTER pAd,
	IN PUCHAR pIpHeader)
{
	UINT32 ipAddr = 0, idx = 0;
	char *special_ip_list[] = {
	...
	while (strlen(special_ip_list[idx]) > 0)
	{
		ipAddr = in_aton(special_ip_list[idx]);
		if (ipAddr == (*((UINT32 *)(pIpHeader + 16))))
		{
			return TRUE;
		}
		++idx;
	...
	if (IS_MULTICAST_MAC_ADDR(pSrcBufVA))
	{
		...
		if (IgmpMldPkt)
		{
			...
			*pInIgmpGroup = IGMP_PKT;
									...
									wdev->if_dev)) == NULL)
				&& !isInvalidM2UIP(pAd, pSrcBufVA + 14))	/* yanglx, 2014-9-18: ported from MT7610E */
		{
			RELEASE_NDIS_PACKET(pAd, pPacket, NDIS_STATUS_FAILURE);
			return NDIS_STATUS_FAILURE;
		}
