...
{
	DBGPRINT(RT_DEBUG_TRACE, ("cfg_mode = %d\n", cfg_mode));
       ...
       /* Set some preconfigured value before interface up*/
		...
		case CMD_RTPRIV_IOCTL_ATE:
			...
			case RTPRIV_IOCTL_ATE_SET : 
				...
				break;
			case RTPRIV_IOCTL_ATE_AUTO_ALC :
				Set_ATE_AUTO_ALC_Proc(pAd, (PSTRING)wrq->u.data.pointer);
				break;
			case RTPRIV_IOCTL_ATE_IPG:
				Set_ATE_IPG_Proc(pAd, (PSTRING)wrq->u.data.pointer);
				break;
			case RTPRIV_IOCTL_ATE_SET_TX_POW_EVAL:
				Set_ATE_TX_POWER_EVALUATION_Proc(pAd, (PSTRING)wrq->u.data.pointer);
				break;
			default :
				RtmpDoAte(pAd, wrq, pData);
			}
