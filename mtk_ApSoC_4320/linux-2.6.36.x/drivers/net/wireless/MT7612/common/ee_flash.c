...
#define RF_FIRST_OFFSET 	CONFIG_RT_FIRST_IF_RF_OFFSET
#define RF_SECOND_OFFSET	CONFIG_RT_SECOND_IF_RF_OFFSET
...
#else
	//RtmpFlashWrite(pAd->eebuf, pAd->flash_offset, EEPROM_SIZE);
	rtmp_ee_write_flash(pAd, NULL);
#endif /* MULTIPLE_CARD_SUPPORT */
}

VOID rtmp_ee_write_flash(
	PRTMP_ADAPTER	pAd, 
	PSTRING			arg)
{
	UCHAR *buf;
	...
	os_alloc_mem(NULL, (UCHAR **)&buf, sizeof(UCHAR)*EEPROM_SIZE_ALL);
	if (buf == NULL)
	{
		printk("\n####  malloc fail !! ####\n");
	}
	...
	{
		printk("\n####  write 5G cal to flash !! ####\n");

		/* flash op's size and offset must be n*64K */
		spiflash_ioctl_read(buf, RF_FIRST_OFFSET, EEPROM_SIZE_ALL);
		spiflash_ioctl_erase(RF_FIRST_OFFSET, EEPROM_SIZE_ALL);
		for (index = 0; index < EEPROM_SIZE; index++)
		{	/* only update second rf part */
			buf[RF_SECOND_OFFSET - RF_FIRST_OFFSET + index] = pAd->eebuf[index];
		}
		spiflash_ioctl_write(buf, RF_FIRST_OFFSET, EEPROM_SIZE_ALL);
	}
...
static NDIS_STATUS rtmp_ee_flash_reset(RTMP_ADAPTER *pAd, UCHAR *start)
{
	...
	{
		/*printk("default EEPROM file path = %s\n", EEPROM_DEFAULT_FILE_PATH);*/
...
static NDIS_STATUS rtmp_ee_flash_init(PRTMP_ADAPTER pAd, PUCHAR start)
{
	...
	if (validFlashEepromID(pAd) == FALSE)
	{
		...
		/* Random number for the last bytes of MAC address*/
		{
			...
			rtmp_ee_flash_write(pAd, 0x08, Addr45);
			DBGPRINT(RT_DEBUG_ERROR, ("%s: The EEPROM in Flash is wrong, use default\n", __func__));
		}
...
*/
    ...
    spiflash_ioctl_read(pAd->eebuf, pAd->flash_offset, EEPROM_SIZE);
#endif /* MULTIPLE_CARD_SUPPORT */
