...
void send_reset_to_user(PWSC_CREDENTIAL pWscCredential)
{
	...
	int ret = 0;
	char buf[BUF_SIZE_RTNL] = "WLAN 5G WPS";
	NdisMoveMemory(buf + strlen("WLAN 5G WPS"), pWscCredential, len);
	len += strlen("WLAN 5G WPS");
	...
	if (CurOpMode == AP_MODE)
	{
		if ((OpMode & 0x0F) == AP_MODE)
		{
				...
				(pWscControl->WscDefaultSsid.SsidLength < 33)*/)
			{
				...
				char ssid[MAX_LEN_OF_SSID] = {0};
				UCHAR tempByte[10] = {0};
				int index = 0;
				for (index = 0; index < 3; index++)
				{
					tempByte[index] = RandomByte(pAd);
				}
...
// Imported by yanglx from MT7610E, 2014-10-28.
...
extern void register_simple_config_callback_5G (void *callback, void *arg);
irqreturn_t WSC_HDR_BTN_HANDLER_5G(int irq, void *pAd, struct pt_regs *regs)
{
#ifdef CONFIG_AP_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	register_simple_config_callback_5G(WSC_HDR_BTN_HANDLER_5G, pAd);
