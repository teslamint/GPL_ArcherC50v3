...
static unsigned short csum_update(unsigned short old_csum,
...
static unsigned short csum_update_ip(unsigned short old_check,
...
#define FIRST_EEPROM_FILE_PATH	"/var/Wireless/RT2860AP/"
#if defined(CONFIG_SUPPORT_OPENWRT)
#define FIRST_AP_PROFILE_PATH		"/etc/wireless/mt7602e/mt7602e.dat"
#else
#define FIRST_AP_PROFILE_PATH		"/var/Wireless/RT2860AP/RT2860AP5G.dat"
#endif /* CONFIG_SUPPORT_OPENWRT */
#define FIRST_CHIP_ID	xdef_to_str(CONFIG_RT_FIRST_CARD)

#define SECOND_EEPROM_FILE_PATH	"/var/Wireless/RT2860AP/"
#if defined(CONFIG_SUPPORT_OPENWRT)
#define SECOND_AP_PROFILE_PATH	"/etc/wireless/mt7612e/mt7612e.dat"
#else
#define SECOND_AP_PROFILE_PATH	"/var/Wireless/RT2860AP/RT2860AP5G.dat"
#endif /* CONFIG_SUPPORT_OPENWRT */
...
NDIS_STATUS	RTMPReadParametersHook(RTMP_ADAPTER *pAd)
{
	...
	PSTRING	src_tmp = NULL;
	RTMP_OS_FD srcf_tmp;
	INT retval = NDIS_STATUS_FAILURE, ret = -1;
	...
	if (src && *src)
	{
		...
		{
			DBGPRINT(RT_DEBUG_ERROR, ("%s: Open file \"%s\" failed!\n", __func__, src));

			/* add by panxuzhou for default config */
			src_tmp = AP_PROFILE_PATH_DEFAULT;
			srcf_tmp = RtmpOSFileOpen(src_tmp, O_RDONLY, 0);
			if (!IS_FILE_OPEN_ERR(srcf_tmp))
			{
				...
				if (buffer) 
				{
					...
					ret = RtmpOSFileRead(srcf_tmp, buffer, MAX_INI_BUFFER_SIZE);
					if (ret > 0)
					{
						RTMPSetProfileParameters(pAd, buffer);
						DBGPRINT(RT_DEBUG_ERROR, ("%s: try default profile \"%s\" ok!\n", __func__, src_tmp));
	...
	IN UCHAR OpMode)
{
	...
	struct sk_buff * rx_skb = RTPKT_TO_OSPKT(pRxPkt);
	if (kSdmzObj.enable)
	{
		struct ethhdr *eth;
		struct udphdr *uhs;
		...
		{
			rx_skb->network_header = rx_skb->mac_header + ETH_HLEN;
			iphs = ip_hdr(rx_skb);
...
#ifdef CONFIG_TP_SUPER_DMZ
	extern struct K_SDMZ_OBJ kSdmzObj;
	struct sk_buff * tx_skb = RTPKT_TO_OSPKT(pPacket);
	if (kSdmzObj.enable)
	{
		struct ethhdr *eth;
		struct udphdr *uhs;
		...
		if (eth->h_proto == htons(ETH_P_IP))
		{
			tx_skb->network_header = tx_skb->mac_header + ETH_HLEN;
			iphs = ip_hdr(tx_skb);
			tx_skb->transport_header = tx_skb->network_header + iphs->ihl * 4;
