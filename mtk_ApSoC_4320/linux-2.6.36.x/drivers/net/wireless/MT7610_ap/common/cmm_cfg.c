...
INT ed_status_read(RTMP_ADAPTER *pAd)
{
	...
	UINT32 ch_idle_stat=0, ch_busy_stat=0, ed_2nd_stat=0, ed_stat=0, pkt_cnt=0;
	
	RTMP_IO_READ32(pAd, CH_IDLE_STA, &ch_idle_stat);
	RTMP_IO_READ32(pAd, 0x1140, &ed_stat);
	//get pkt to me cnt
	RTMP_IO_READ32(pAd, 0x1504, &pkt_cnt);
	...
	pAd->ed_pkt_cnt[pAd->ed_stat_lidx] = pkt_cnt;
	...
	if ((pAd->ed_threshold > 0) && (period_us > 0) && (pAd->ed_block_tx_threshold > 0)) {
		...
		//when every 100ms pkt cnt < pAd->ed_pktcnt_th,maybe not in t-put mode
		if ( pkt_cnt > pAd->ed_pktcnt_th )
		{
			pAd->ed_pkt_trigger_cnt =0;
		}
		else
		{
			pAd->ed_pkt_trigger_cnt ++;
		}
	...
	RTMP_IRQ_UNLOCK(&pAd->irq_lock, irqflag);
		...
		{
			if (pAd->ed_trigger_cnt > pAd->ed_block_tx_threshold && pAd->ed_pkt_trigger_cnt > pAd->ed_block_tx_threshold)  {
...
{
	UINT ed_pktcnt_th = simple_strtol(arg, 0, 10);

	pAd->ed_pktcnt_th = ed_pktcnt_th;
	DBGPRINT(RT_DEBUG_OFF, ("%s(): ed_pktcnt_th=%d\n", 
		__FUNCTION__, pAd->ed_pktcnt_th));
...
INT show_ed_stat_proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	...
	NdisMoveMemory(&ed_pkt_cnt[0], &pAd->ed_pkt_cnt[0], sizeof(ed_pkt_cnt));
	...
	idx = start;
	do {
		DBGPRINT(RT_DEBUG_OFF, ("\t%d", ed_pkt_cnt[idx]));
		INC_RING_INDEX(idx, ED_STAT_CNT);
	} while (idx != end);
