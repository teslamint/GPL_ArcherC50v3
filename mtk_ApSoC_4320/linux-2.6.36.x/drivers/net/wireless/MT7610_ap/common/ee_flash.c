...
#endif
		/* Do not write it into flash. Use rtmp_ee_flash_write_all instead.*/
		DBGPRINT(RT_DEBUG_OFF, ("write cal to mem only!! \n"));
	}
	return 0;
}
...
VOID rtmp_ee_flash_write_all(PRTMP_ADAPTER pAd, USHORT *Data)
{
	...
	RtmpFlashWrite(pAd->chipCap.eebuf, RF_OFFSET, EEPROM_SIZE);
#endif
	/* Write all eeprom data in memory into flash. */
	...
	else
	{
		...
		spiflash_ioctl_read(buf, DEFAULT_WHOLE_RF_OFFSET, EEPROM_SIZE_ALL);
		spiflash_ioctl_erase(DEFAULT_WHOLE_RF_OFFSET, EEPROM_SIZE_ALL);
		for (index = 0; index < EEPROM_SIZE; index++)
		{
			buf[index + DEFAULT_RF_OFFSET -  DEFAULT_WHOLE_RF_OFFSET] = pAd->chipCap.eebuf[index];
		}
		spiflash_ioctl_write(buf, DEFAULT_WHOLE_RF_OFFSET, EEPROM_SIZE_ALL);
	}
...
#endif
#endif
	/* read all flash data into memory */
	DBGPRINT(RT_DEBUG_OFF, ("Read eeprom data from flash into memory!\n"));
	spiflash_ioctl_read(pAd->chipCap.eebuf, DEFAULT_RF_OFFSET, EEPROM_SIZE);	
