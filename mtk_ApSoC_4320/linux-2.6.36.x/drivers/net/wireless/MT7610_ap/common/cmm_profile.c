...
#ifdef WSC_V2_SUPPORT
#ifdef WSC_AP_SUPPORT
		if (RTMPGetKeyParameter("WscSetupLock", tmpbuf, 10, pBuffer, TRUE))
		{
			for (i = 0; i < pAd->ApCfg.BssidNum; ++i)
			{
				pAd->ApCfg.MBSSID[i].WscControl.bSetupLock = simple_strtol(tmpbuf, 0, 10);
			}
		}
#endif /* WSC_V2_SUPPORT */
#endif /* WSC_AP_SUPPORT */
...
#ifdef SINGLE_SKU_V2
#define ENABLE_SKU_POWER_HALF 1
#ifdef ENABLE_SKU_POWER_HALF
/* Check if the token is a half of an int. 
 * For example, if the power is 15.5, the result of simple_strtod()*2 is 30, so we should add 1 to the result.
 */
static int token_is_half(const char *cp)
{
	...
	IN RTMP_ADAPTER *pAd)
{
	...
	/* init*/
	os_alloc_mem(NULL, (UCHAR **)&buffer, MAX_SKU_BUFFER_SIZE);
	if (buffer == NULL)
		return FALSE;
	...
	/* card information file exists so reading the card information */
	NdisZeroMemory(buffer, MAX_SKU_BUFFER_SIZE);
	retval = RtmpOSFileRead(srcf, buffer, MAX_SKU_BUFFER_SIZE);
	if (retval < 0)
	{
		/* read fail */
		...
		for ( readline = ptr = buffer, index=0; (ptr = strchr(readline, '\n')) != NULL; readline = ptr + 1, index++ )
		{
			*ptr = '\0';
			...
			if ( !strncmp(readline, "ch", 2) )
			{
				...
				if (pwr->PwrVHT20[8] == 0)
					{
					/* VHT20 MCS8 power can't be 0. It should be an old SKU table. Just fill VHT20 with HT20 to be compatible with old SKU.*/
					for ( i= 0 ; i < SINGLE_SKU_TABLE_HT_LENGTH >> 1 ; i++ )
						{
						pwr->PwrVHT20[i] = pwr->PwrHT20[i];
						}
					pwr->PwrVHT20[8] = pwr->PwrVHT80[8];
					}
				
				if (pwr->PwrVHT40[9] == 0)
					{
					/* VHT40 MCS9 power can't be 0. It should be an old SKU table. Just fill VHT40 with HT40  to be compatible with old SKU..*/
					for ( i= 0 ; i < SINGLE_SKU_TABLE_HT_LENGTH >> 1 ; i++ )
						{
						pwr->PwrVHT40[i] = pwr->PwrHT40[i];
						}
					pwr->PwrVHT40[8] = pwr->PwrVHT80[8];
					pwr->PwrVHT40[9] = pwr->PwrVHT80[9];
					}
#endif/*#ifdef ENABLE_SKU_VHT20_VHT40*/
				{
					StartCh = pwr;
...
#if 0
	{
		...
		DlListForEachSafe(ch, ch_temp, &pAd->SingleSkuPwrList, CH_POWER, List)
		{
			...
			for ( i= 0 ; i < SINGLE_SKU_TABLE_VHT20_LENGTH ; i++ )
			{
				printk("%d ", ch->PwrVHT20[i]);
			}
			printk("\n");

			for ( i= 0 ; i < SINGLE_SKU_TABLE_VHT_LENGTH ; i++ )
			{
				printk("%d ", ch->PwrVHT40[i]);
			}
			printk("\n");
