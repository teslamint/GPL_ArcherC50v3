...
INT set_ed_pkt_cnt_threshold(RTMP_ADAPTER *pAd, PSTRING arg);
...
static struct {
	...
	*/
	...
	{"ed_pkt_cnt", set_ed_pkt_cnt_threshold},
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	INT apidx = pObj->ioctl_if;
	INT mcs = pAd->ApCfg.MBSSID[apidx].DesiredTransmitSetting.field.MCS;
	INT i, j=0;
...
#define RSSI_BASE (-96)
	...
	for(i=0; i<pAd->ScanTab.BssNr ;i++)
	{
		...
		if( (pAd->CommonCfg.CountryRegionForABand== 0) && (pBss->Channel > 48) && (pBss->Channel < 149) )	//FCC
		{
			continue;
		}
		
		if( (pAd->CommonCfg.CountryRegionForABand == 1) && (pBss->Channel > 48) )	//CE
		{
			continue;
		}

		pBssInfo[j].channel = (unsigned int)pBss->Channel;
		rssi_db = pBss->Rssi;
		rssi_dbm = rssi_db - RSSI_BASE;
		pBssInfo[j].rssi = (unsigned int)rssi_dbm;
		//printk("Bss %s auth is %d, wepStatus is %d\n", pBss->Ssid, pBss->AuthMode, pBss->WepStatus);
		if (FALSE == pBss->Privacy)
		{
			pBssInfo[j].securityEnable = FALSE;
		}
		else
		{
			pBssInfo[j].securityEnable = TRUE;
		}
		pBssInfo[j].authMode = pBss->AuthMode;
		pBssInfo[j].wepStatus = pBss->WepStatus;

		memcpy(pBssInfo[j].ssid, pBss->Ssid, MAX_LEN_OF_SSID);
		memcpy(pBssInfo[j].bssid, pBss->Bssid, MAC_ADDR_LEN);
		j++;
	}
