	...
	IN BOOLEAN bSave)
{
	...
	doCal2 = doCal1 & 0x10;
	DBGPRINT(RT_DEBUG_TRACE,("%s():  docal = [%04x] valid bit[%x]\n",__FUNCTION__,doCal1,doCal2));
	...
	IN UCHAR 			channel)
{
	...
	DlListForEachSafe(ch, ch_temp, &pAd->SingleSkuPwrList, CH_POWER, List)
	{
		...
		if (channel == ch->channel)
		{
			...
			/* We can't distinguish 11n and 11ac here. Just use all of them to decide the base power.*/
#ifndef ENABLE_SKU_VHT20_VHT40
			/* VHT 20 MCS8 use VHT80 MCS8 power */
			if (base_pwr > ch->PwrVHT80[8])
				base_pwr = ch->PwrVHT80[8];
#else
			for ( i= 0 ; i < SINGLE_SKU_TABLE_VHT20_LENGTH ; i++ )
			{
				if ( ch->PwrVHT20[i] == 0 )
						break;
				if ( base_pwr > ch->PwrVHT20[i] )
					base_pwr = ch->PwrVHT20[i];
			}
#endif	
			//printk("warning:::use VHT BW %d, mode %d\n", pAd->CommonCfg.MlmeTransmit.field.BW, pAd->CommonCfg.MlmeTransmit.field.MODE);
			if (pAd->CommonCfg.BBPCurrentBW == BW_40)
			{
...
#ifndef ENABLE_SKU_VHT20_VHT40
				/* VHT 40 MCS8,MCS9 use VHT80 MCS8, MCS9 power */
				if (base_pwr > ch->PwrVHT80[8])
					base_pwr = ch->PwrVHT80[8];
				if (base_pwr > ch->PwrVHT80[9])
					base_pwr = ch->PwrVHT80[9];
#else
				for ( i= 0 ; i < SINGLE_SKU_TABLE_VHT_LENGTH ; i++ )
				{
					...
					if ( base_pwr > ch->PwrVHT40[i] )
						base_pwr = ch->PwrVHT40[i];
				}
#endif
			}
	...
	IN UCHAR 			channel)
{
	...
	*/
	...
	DBGPRINT(RT_DEBUG_OFF, ("%s ==> pAd->CommonCfg.BBPCurrentBW = %d\n", 
	...
	DlListForEachSafe(ch, ch_temp, &pAd->SingleSkuPwrList, CH_POWER, List)
	{
		...
		if (channel == ch->channel)
		{
			...
			for (i = 0; i < SINGLE_SKU_TABLE_OFDM_LENGTH; i++)
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s ==> OFDM[%d].MCS_Power = %d, ch_init_pwr = %d, ch_delta_pwr = %d\n",
					__FUNCTION__, i, pAd->chipCap.rate_pwr_table.OFDM[i].MCS_Power, ch_init_pwr, ch_delta_pwr));
			...
			for (i = 0; i < (SINGLE_SKU_TABLE_HT_LENGTH >> 1); i++)
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s ==> HT[%d].MCS_Power = %d, ch_init_pwr = %d, ch_delta_pwr = %d\n", 
					__FUNCTION__, i, pAd->chipCap.rate_pwr_table.HT[i].MCS_Power, ch_init_pwr, ch_delta_pwr));
...
#endif
			{
				...
				DBGPRINT(RT_DEBUG_TRACE, ("%s ==> VHT[%d].MCS_Power = %d, ch_init_pwr = %d, ch_delta_pwr = %d\n", 
					__FUNCTION__, i, pAd->chipCap.rate_pwr_table.VHT[i].MCS_Power, ch_init_pwr, ch_delta_pwr));

				rate_pwr_before_adjust = pAd->chipCap.rate_pwr_table.VHT[i].MCS_Power + DefaultTargetPwr;
				rate_pwr =  rate_pwr_before_adjust + ch_delta_pwr;
#ifndef ENABLE_SKU_VHT20_VHT40
				/* for VHT20 VHT40 VHT80 MCS8 and MCS9, use VHT80 MCS8 and MCS9 */
				if (i >=8))
				{
					sku_pwr = (ch->PwrVHT80[i] > rate_pwr_before_adjust) ? \
								rate_pwr_before_adjust : ch->PwrVHT80[i];
				...
				if (pAd->CommonCfg.BBPCurrentBW == BW_20)
				{
					...
					if (i >= 8)
						{
						/* VHT 20 should not go beyond MCS8, just use MCS8 */ 
						sku_pwr = (ch->PwrVHT20[i] > rate_pwr_before_adjust) ? \
								rate_pwr_before_adjust : ch->PwrVHT20[8];
						}
					else
						{
					sku_pwr = (ch->PwrVHT20[i] > rate_pwr_before_adjust) ? \
								rate_pwr_before_adjust : ch->PwrVHT20[i];
						}
				#endif
				}
				else if (pAd->CommonCfg.BBPCurrentBW == BW_40)
				{
					...
					sku_pwr = (ch->PwrHT40[i] > rate_pwr_before_adjust) ? \
								rate_pwr_before_adjust : ch->PwrHT40[i];
				#else
					sku_pwr = (ch->PwrVHT40[i] > rate_pwr_before_adjust) ? \
								rate_pwr_before_adjust : ch->PwrVHT40[i];
				#endif
				}
				else if (pAd->CommonCfg.BBPCurrentBW == BW_80)
				{
