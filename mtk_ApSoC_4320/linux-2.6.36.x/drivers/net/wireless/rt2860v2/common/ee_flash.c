...
VOID rtmp_ee_flash_write_all(PRTMP_ADAPTER pAd, USHORT *Data)
{
	...
	if (NULL == buf)
		{
		DBGPRINT(RT_DEBUG_OFF, ("mallco error!"));
		}
	else
		{
		DBGPRINT(RT_DEBUG_OFF, ("\n####  write 2.4G cal to flash !! ####\n"));
		//RtmpFlashWrite(pAd->eebuf, RF_OFFSET, EEPROM_SIZE);
		spiflash_ioctl_read(buf, RF_OFFSET, EEPROM_SIZE_ALL);
		spiflash_ioctl_erase(RF_OFFSET, EEPROM_SIZE_ALL);
		for (index = 0; index < EEPROM_SIZE; index++)
		{
			buf[index] = pAd->eebuf[index];
		}
		spiflash_ioctl_write(buf, RF_OFFSET, EEPROM_SIZE_ALL);
		}
...
/*	ASSERT((pAd->eebuf == NULL)); */
	...
	spiflash_ioctl_read(pAd->eebuf, RF_OFFSET, EEPROM_SIZE);
#endif /* MULTIPLE_CARD_SUPPORT */
