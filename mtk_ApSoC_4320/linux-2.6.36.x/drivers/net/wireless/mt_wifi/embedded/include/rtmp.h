...
VOID TP_SetWSCLedStatus(
unsigned int WSCLedStatus);

VOID TP_CheckWSCLEDStatus(
PWSC_CTRL pWscCtrl);
//end added
#endif
...
#define MAX_GROUP_ID_LEN    14         /* IPv6's group id is 112 bits */
typedef struct _MULTICAST_GROUP_ID {
    UCHAR data[MAX_GROUP_ID_LEN];
} MULTICAST_GROUP_ID, *PMULTICAST_GROUP_ID;

#define GID_HASH(gid)\
    (gid.data[0] ^ gid.data[1] ^ gid.data[2] ^ gid.data[3] ^ gid.data[4] ^ gid.data[5] ^ gid.data[6] ^\
    gid.data[7] ^ gid.data[8] ^ gid.data[9] ^ gid.data[10] ^ gid.data[11] ^ gid.data[12] ^ gid.data[13])
...
#define GROUP_ID_EQUAL(gid1, gid2) RTMPEqualMemory((PVOID)((gid1).data), (PVOID)((gid2).data), MAX_GROUP_ID_LEN)

static inline VOID getGroupIDFromIPAddr(
	IN PUCHAR pIpAddr,
	IN PMULTICAST_GROUP_ID pGroupId,
	IN UINT16 ProtoType)
{
	if (pIpAddr == NULL)
		return;
...
/*			memset(*ppMacAddr, 0, MAC_ADDR_LEN); */
            memcpy(pGroupId->data, pIpAddr + 2, MAX_GROUP_ID_LEN);
			break;

		case ETH_P_IP:
		default:
            memcpy(pGroupId->data, pIpAddr, 4);
			memset(pGroupId->data + 4, 0, MAX_GROUP_ID_LEN - 4);
			break;
	}
