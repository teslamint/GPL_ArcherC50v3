...
static VOID mt_FillSkuParameter(RTMP_ADAPTER *pAd,UINT8 channel,struct _EXT_CMD_CHAN_SWITCH_T *pCmdChanSwitch)
{
    CH_POWER *ch, *ch_temp;
    UCHAR start_ch;
    UINT8 j;
    UINT8 *txPowerSku = pCmdChanSwitch->aucTxPowerSKU;
    UINT8 tempChannel = channel;
	int ucBW = pCmdChanSwitch->ucBW;
	int ucCtrlCh = pCmdChanSwitch->ucCtrlCh;
	int ucCentralCh = pCmdChanSwitch->ucCentralCh;
	printk("\ncurrent bandwidth %d, ctrl channel %d, central channel %d\n", ucBW,ucCtrlCh, ucCentralCh);

	int index = 0;
    DlListForEachSafe(ch, ch_temp, &pAd->SingleSkuPwrList, CH_POWER, List)
    {
        start_ch = ch->StartChannel;
        if ( BW_20 == pCmdChanSwitch->ucBW )
        {
            if ( tempChannel >= start_ch )
            {
                for ( j = 0; j < ch->num; j++ )
                {
                    if ( tempChannel == ch->Channel[j] )
                    {
			...
			for ( index =SKU_CCK_1_2;index < SKU_HT40_7_15;index ++)
				{
				 MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: txPowerSku[%d] = %d\n", __FUNCTION__,index ,txPowerSku[index]));
				}
                        break;
                    }
                }
        ...
        if ( BW_40 == pCmdChanSwitch->ucBW )
        {
            if ( pCmdChanSwitch->ucCtrlCh >= start_ch)
            {
 
                for ( j = 0; j < ch->num; j++ )
                {
                    if ( pCmdChanSwitch->ucCtrlCh == ch->Channel[j] )
                    {
                        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\nFind the right list in SingleSkuPwrList, start_ch:%d, ch_num: %d\n", start_ch, ch->num));
                        txPowerSku[SKU_CCK_1_2]     = ch->PwrCCK[0] ? ch->PwrCCK[0] : 0xff;
                        txPowerSku[SKU_CCK_55_11]     = ch->PwrCCK[2] ? ch->PwrCCK[2] : 0xff;
                        txPowerSku[SKU_OFDM_6_9]     = ch->PwrOFDM[0] ? ch->PwrOFDM[0] : 0xff;
                        txPowerSku[SKU_OFDM_12_18]= ch->PwrOFDM[2] ? ch->PwrOFDM[2] : 0xff;
						...
						for ( index =SKU_CCK_1_2;index < SKU_HT20_7_15;index ++)
				{
				 MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: txPowerSku[%d] = %d\n", __FUNCTION__,index ,txPowerSku[index]));
				}
            ...
            if ( pCmdChanSwitch->ucCentralCh >= start_ch)
            {
 
                for ( j = 0; j < ch->num; j++ )
                {
                    if ( pCmdChanSwitch->ucCentralCh == ch->Channel[j] )
                    {
                        MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\nFind the right list in SingleSkuPwrList, start_ch:%d, ch_num: %d\n", start_ch, ch->num));
                        txPowerSku[SKU_HT40_0_8]     = ch->PwrHT40[0] ? ch->PwrHT40[0]  : 0xff;
                        txPowerSku[SKU_HT40_32]     = 0xff;
                        ...
                        txPowerSku[SKU_HT40_6_14]     = ch->PwrHT40[6] ?  ch->PwrHT40[6] : 0xff;
                        txPowerSku[SKU_HT40_7_15]     = ch->PwrHT40[7] ?  ch->PwrHT40[7] : 0xff;
				for ( index =SKU_HT40_0_8;index < SKU_HT40_7_15;index ++)
				{
				 MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: txPowerSku[%d] = %d\n", __FUNCTION__,index ,txPowerSku[index]));
				}
...
#ifdef SINGLE_SKU_V2
	if  (pAd->SKUEn)
	mt_FillSkuParameter(pAd,central_chl,&CmdChanSwitch);
#endif
...
{
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("FW LOG: %s\n", Data));
}
