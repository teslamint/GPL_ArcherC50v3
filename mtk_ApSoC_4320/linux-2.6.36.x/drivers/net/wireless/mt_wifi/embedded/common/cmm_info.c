...
#define	CHANNEL_LIST_LEN	(1+1+61+1)	/* "\n"+num+channel_list+"\n" */
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	os_alloc_mem(NULL, (UCHAR **)&msg, sizeof(CHAR)*CHANNEL_LIST_LEN);
	if (msg == NULL)
	{
	...
	NdisZeroMemory(msg, sizeof(CHAR)*CHANNEL_LIST_LEN);
	sprintf(msg, "%s", "\n");
	sprintf(msg+strlen(msg), "%c", (int)pAd->ChannelListNum);
	...
	for (i=0; i < pAd->ChannelListNum; i++)
	{
		...
		sprintf(msg+strlen(msg), "%c", (int)pAd->ChannelList[i].Channel);
	}

	sprintf(msg+strlen(msg), "%s", "\n");

	/* for compatible with old API just do the printk to console*/
	wrq->u.data.length = CHANNEL_LIST_LEN;
	if (copy_to_user(wrq->u.data.pointer, msg, wrq->u.data.length))
	{
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: copy_to_user() fail\n", __FUNCTION__));
	}
 ...
 INT RTMPIoctlConnStatus(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("=============================================================\n"));
 		...
 		&& (pAd->ApCfg.ApCliTab[ifIndex].SsidLen != 0))
 	{
 		for (i=0; i<MAX_LEN_OF_MAC_TABLE; i++)
 		{
 			...
 			if ( IS_ENTRY_APCLI(pEntry)
				...
				&& (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED))
 				{
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApCli%d Connected AP : %02X:%02X:%02X:%02X:%02X:%02X   SSID:%s\n",
							ifIndex, PRINT_MAC(pEntry->Addr), pAd->ApCfg.ApCliTab[ifIndex].Ssid));
                    #if 1
					sprintf(arg+strlen(arg),"%-7d", pEntry->RssiSample.AvgRssi[0]);
					sprintf(arg+strlen(arg),"%-7d", pEntry->RssiSample.AvgRssi[1]);
					sprintf(arg+strlen(arg),"%-7d", pEntry->RssiSample.AvgRssi[2]);
					sprintf(arg+strlen(arg),"\n");

					sprintf(arg+strlen(arg), "ApCli%d Connected AP : %02X:%02X:%02X:%02X:%02X:%02X   SSID:%s\n",ifIndex,
							PRINT_MAC(pEntry->Addr),
							pAd->ApCfg.ApCliTab[ifIndex].Ssid);
                    #endif
                    MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApCli%d Connected AP : %02X:%02X:%02X:%02X:%02X:%02X   SSID:%s, avgrssi: %-7d, %-7d, %-7d\n",
							ifIndex, PRINT_MAC(pEntry->Addr), pAd->ApCfg.ApCliTab[ifIndex].Ssid, pEntry->RssiSample.AvgRssi[0],pEntry->RssiSample.AvgRssi[1],pEntry->RssiSample.AvgRssi[2]));
					bConnect=TRUE;
 				}
 		}
		...
		if (!bConnect)
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApCli%d Connected AP : Disconnect\n",ifIndex));
			sprintf(arg, "ApCli%d Connected AP : Disconnect\n", ifIndex);
		}
 	}
 	else
 	{
...
INT Show_Cfg_Value_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
    RTMP_STRING buf[1024], *pBuf;
    UINT32	MaxLen = 1024;

    pBuf = (RTMP_STRING *)&buf;
    RTMPZeroMemory(pBuf, 1024);

    if (!RTMPShowCfgValue(pAd, arg, pBuf, MaxLen))
    {
        printk("%s Is:\n%s\n", arg, buf);
    }
