...
//added by caizifeng for wps led
static int TP_WSCLedTimeout = 0;	// Control Led flash Time
static unsigned int TP_WSCLedStatus_old = TP_WSC_LED_END;
unsigned int TP_WSCLEDStatus = TP_WSC_LED_END;

#define TP_LED_HZ (1000 / MLME_TASK_EXEC_INTV)
#define TP_WSC_LED_SUCCESS_TIMEOUT (300 * TP_LED_HZ)
#define TP_WSC_LED_ERROR_TIMEOUT (30 * TP_LED_HZ)
#define TP_WSC_LED_PBCOVERLAPED_TIMEOUT (120 * TP_LED_HZ)

extern void led_setWpsOn(void);
extern void led_setWpsOff(void);

static inline VOID TP_setWSCLEDOff(void)
{
	//ledTurnOff(LED_WLAN_WPS_STATUS);
	led_setWpsOff();
}

static inline VOID TP_setWSCLEDOn(void)
{
	//ledTurnOn(LED_WLAN_WPS_STATUS);
	led_setWpsOn();
}

VOID TP_SetWSCLedStatus(unsigned int WSCLedStatus)
{
	// ??WSC??????????3?????300s????????????END????????????.?????????????????????????
	if (TP_WSCLEDStatus == TP_WSC_LED_SUCCESS && WSCLedStatus == TP_WSC_LED_END)
	{
		return;
	}
	TP_WSCLEDStatus = WSCLedStatus;
	//printk("%s : %d\n", __FUNCTION__, WSCLedStatus);
	switch (TP_WSCLEDStatus)
	{
	case TP_WSC_LED_END:
	case TP_WSC_LED_START:
		TP_WSCLedTimeout = 0;
		break;
	case TP_WSC_LED_PBC_OVERLAPPED:
		TP_WSCLedTimeout = TP_WSC_LED_PBCOVERLAPED_TIMEOUT;
		break;
	case TP_WSC_LED_ERROR:
		TP_WSCLedTimeout = TP_WSC_LED_ERROR_TIMEOUT;
		break;
	case TP_WSC_LED_SUCCESS:
		TP_WSCLedTimeout = TP_WSC_LED_SUCCESS_TIMEOUT;
		break;
	case TP_WSC_LED_NOP:
	default:
		TP_WSCLedTimeout = 1;
...
VOID TP_CheckWSCLEDStatus(PWSC_CTRL pWscCtrl)
{
	static unsigned int cnt = 0;
	//printk("*****tpcheckWscledstatus blink\n");
	if (TP_WSCLedTimeout > 0 && --TP_WSCLedTimeout <= 0)
	{
		TP_WSCLEDStatus = TP_WSC_LED_END;
	}

	// Detecte if wsc is processing
	if ((pWscCtrl->WscStatus >= STATUS_WSC_LINK_UP && STATUS_WSC_START_ASSOC >= pWscCtrl->WscStatus && pWscCtrl->WscStatus != STATUS_WSC_CONFIGURED)
	  && TP_WSCLedStatus_old != TP_WSC_LED_START)
	{
		if (pWscCtrl->WscActionMode != WSC_ENROLLEE && pWscCtrl->WscStatus != STATUS_WSC_EAP_M1_RECEIVED)
		TP_SetWSCLedStatus(TP_WSC_LED_START);
	}
	else if (pWscCtrl->WscStatus == STATUS_WSC_PBC_SESSION_OVERLAP && TP_WSCLedStatus_old != TP_WSC_LED_PBC_OVERLAPPED)
	{
		TP_SetWSCLedStatus(TP_WSC_LED_PBC_OVERLAPPED);
	}
	
	if (TP_WSCLedStatus_old == TP_WSCLEDStatus)
	{
		if (TP_WSC_LED_END == TP_WSCLedStatus_old)
			return;
	}
	else
	{
		cnt = 0;
		TP_WSCLedStatus_old = TP_WSCLEDStatus;
	}
#if 0	/* remove dbg print */
	if (TP_WSCLEDStatus == TP_WSC_LED_ERROR)
		printk("****TP_WSCLEDStatus is %d\n",TP_WSCLEDStatus);
#endif	
	switch (TP_WSCLEDStatus)
	{
	case TP_WSC_LED_START:
		if(cnt >=3) cnt =0;
		if(cnt == 0) TP_setWSCLEDOn();
		else if(cnt == 2) TP_setWSCLEDOff();
		cnt++;
		break;
	case TP_WSC_LED_END:
		TP_setWSCLEDOff();
...
EXPORT_SYMBOL(TP_CheckWSCLEDStatus);
#endif
...
#ifdef TP_WSC_LED_SUPPORT
	static POS_COOKIE pObj;
	static PWSC_CTRL pWscControl;
