	...
	IN PUCHAR 			pMacAddr)
{
	...
	getSrcIPFromIpPkt(pLayerHdr, &srcIP);

	//printk("in arp dstIp = %d:%d:%d:%d \n", (dstIP>>24) & 0xff, (dstIP>>16) & 0xff, (dstIP>>8) & 0xff, (dstIP>>0) & 0xff);
	//printk("in arp srcIP = %d:%d:%d:%d \n", (srcIP>>24) & 0xff, (srcIP>>16) & 0xff, (srcIP>>8) & 0xff, (srcIP>>0) & 0xff);
	//printk("in arp dstIP&0xffffff = %d,srcIP&0xffffff = %d \n", dstIP&0xFFFFFF, srcIP&0xFFFFFF);
...
/*dumpPkt(RTPKT_TO_OSPKT(pSkb)->data, RTPKT_TO_OSPKT(pSkb)->len); */
	...
	if (isUcastMac && isGoodIP)
	{
		...
		/*if can't find the ip-mac rule and the dstip is in different ip section with srcip, don't change the mac*/
		if ((pRealMac[0] & pRealMac[1] & pRealMac[2] & pRealMac[3] & pRealMac[4] & pRealMac[5])== 0xff && 
			((dstIP&0x00FFFFFF) != (srcIP&0x00FFFFFF)))
		{
				pRealMac = tgtMac;
		}
	}

	/*DBGPRINT(RT_DEBUG_WARN, ("%s(): tgtMac=%02x:%02x:%02x:%02x:%02x:%02x for mapped dstIP(%d.%d.%d.%d)\n", 
					__FUNCTION__, tgtMac[0],tgtMac[1],tgtMac[2],
					tgtMac[3],tgtMac[4],tgtMac[5],
	...
	IN PUCHAR 			pDevMacAdr)
{
	...
	//printk("dstIp = %d:%d:%d:%d \n", (dstIP>>24) & 0xff, (dstIP>>16) & 0xff, (dstIP>>8) & 0xff, (dstIP>>0) & 0xff);
	//printk("srcIP = %d:%d:%d:%d \n", (srcIP>>24) & 0xff, (srcIP>>16) & 0xff, (srcIP>>8) & 0xff, (srcIP>>0) & 0xff);
	//printk("dstIP&0xffffff = %d,srcIP&0xffffff = %d \n", dstIP&0xFFFFFF, srcIP&0xFFFFFF);

    tgtMac = pLayerHdr - 14;
	
	pMacAddr = IPMacTableLookUp(pMatCfg, dstIP); 
	/*if can't find the ip-mac rule and the dstip is in different ip section with srcip, don't change the mac*/
	if ((pMacAddr[0] & pMacAddr[1] & pMacAddr[2] & pMacAddr[3] & pMacAddr[4] & pMacAddr[5])== 0xff &&
		((dstIP&0x00FFFFFF) != (srcIP&0x00FFFFFF)))
	{
		pMacAddr = tgtMac;
	}
	...
	if ((pAd->ApCfg.bMACRepeaterEn) && (pAd->ApCfg.MACRepeaterOuiMode != 1))
	{
		...
		if (pEntry && IS_ENTRY_APCLI(pEntry) && (pEntry->bReptCli == TRUE))
		{
            ...
            /*
             * 修复 bug 153418: RE模式下，RootAP端建立组播源，当RE的多个客户端加入同一组播组时，每个
             *                  客户端下的组播视频都会花屏。
             * 原因是RE使用PSR与RootAP桥接，RE转给RootAP的IGMP报文源MAC地址会被替换成虚拟MAC地址，致
             * 使RootAP的无线组播转单播功能将每个发给RE的组播包复制成多个单播包，每个单播包的目的MAC
             * 地址都是一个虚拟MAC地址，RE在收到这些包时会根据ARP表查询目的IP对应的实际MAC地址，并将
             * 其替换掉虚拟MAC地址。对组播包而言，目的MAC地址就会被设置成该组播IP对应的组播MAC，这样，
             * 每个组播包都被复制成了多份，相互不可区分，造成客户端处理失败。因此，此处需要将目的组播
             * MAC替换回实际的客户端MAC地址。
             *
             * xukuohai@tp-link.net, 12Jan17
             */
            if (IS_MULTICAST_MAC_ADDR(pMacAddr))
            {
                pMacAddr = pReptEntry->OriginalAddress;
            }
...
#endif /* MAC_REPEATER_SUPPORT */
	/*DBGPRINT(RT_DEBUG_WARN, ("%s(): before return. pMacAddr=%02x:%02x:%02x:%02x:%02x:%02x for mapped dstIP(%d.%d.%d.%d)\n", 
					__FUNCTION__, pMacAddr[0],pMacAddr[1],pMacAddr[2],
