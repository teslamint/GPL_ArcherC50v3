...
static void notifyCfgChanged(IN RTMP_ADAPTER *pAd, PWSC_CREDENTIAL pWscCredential);
...
static int rtnetlink_fill_resetinfo(struct sk_buff *skb, int type, char *data, int data_len)
{	
	struct ifinfomsg *r;	
	struct nlmsghdr  *nlh;
...
void send_reset_to_user(PWSC_CREDENTIAL pWscCredential)
{
	unsigned int size = BUF_SIZE_RTNL;
	int len = sizeof(*pWscCredential);
	int ret = 0;
	char buf[BUF_SIZE_RTNL] = "WLAN WPS";
	NdisMoveMemory(buf + strlen("WLAN WPS"), pWscCredential, len);
	len += strlen("WLAN WPS");
	...
	if (rtnetlink_fill_resetinfo(skb, RTM_NEWLINK,
				  buf, len) < 0) 
	{
...
static void notifyCfgChanged(IN RTMP_ADAPTER *pAd, PWSC_CREDENTIAL pWscCredential)
{
	WSC_CREDENTIAL wscCred = *pWscCredential;
	switch (wscCred.AuthType)
	{
	case (WSC_AUTHTYPE_WPAPSK | WSC_AUTHTYPE_WPA2PSK):
		wscCred.AuthType = WLAN_AUTHMODE_PSK_PSK2;
		break;
	case WSC_AUTHTYPE_WPAPSK:
		wscCred.AuthType = WLAN_AUTHMODE_PSK;
		break;
	case WSC_AUTHTYPE_WPA2PSK:
		wscCred.AuthType = WLAN_AUTHMODE_PSK2;
		break;
	case (WSC_AUTHTYPE_WPA | WSC_AUTHTYPE_WPA2):
		wscCred.AuthType = WLAN_AUTHMODE_WPA_WPA2;
		break;
	case WSC_AUTHTYPE_WPA:
		wscCred.AuthType = WLAN_AUTHMODE_WPA;
		break;
	case WSC_AUTHTYPE_WPA2:
		wscCred.AuthType = WLAN_AUTHMODE_WPA2;
		break;
	case (WSC_AUTHTYPE_OPEN | WSC_AUTHTYPE_SHARED):
		wscCred.AuthType = WLAN_AUTHMODE_OPEN_SHARED;
		break;
	case WSC_AUTHTYPE_OPEN:
		wscCred.AuthType = WLAN_AUTHMODE_OPEN;
		break;
	case WSC_AUTHTYPE_SHARED:
	default:
		wscCred.AuthType = WLAN_AUTHMODE_SHARED;
		break;
	}
	switch (wscCred.EncrType)
	{
	case WSC_ENCRTYPE_NONE:
		wscCred.EncrType = WLAN_ENCRYPTMODE_NONE;
		break;
	case (WSC_ENCRTYPE_AES | WSC_ENCRTYPE_TKIP):
		wscCred.EncrType = WLAN_ENCRYPTMODE_TKIP_AES;
		break;
	case WSC_ENCRTYPE_TKIP:
		wscCred.EncrType = WLAN_ENCRYPTMODE_TKIP;
		break;
	case WSC_ENCRTYPE_AES:
		wscCred.EncrType = WLAN_ENCRYPTMODE_AES;
		break;
	case WSC_ENCRTYPE_WEP:
	default:
		wscCred.EncrType = WLAN_ENCRYPTMODE_WEP;
		break;
	}
	send_reset_to_user(&wscCred);
}
...
{		
	...
	else if (MsgType == WSC_MSG_EAP_FAIL)
	{
		...
		else
		{
			...
			TP_SetWSCLedStatus(TP_WSC_LED_ERROR);
#endif			
			}			
#ifdef CONFIG_STA_SUPPORT
			if (CurOpMode == STA_MODE)
...
/*#endif */
...
#endif /* APCLI_SUPPORT */
				{
					...
					if(MsgType == WSC_MSG_WSC_ACK)
					{
						notifyCfgChanged(pAdapter, &pWscControl->WscProfile.Profile[0]);
					}
				}
...
#endif /* WSC_LED_SUPPORT */
#ifdef TP_WSC_LED_SUPPORT
			TP_SetWSCLedStatus(TP_WSC_LED_SUCCESS);
#endif
	}
}
...
#endif /* CONFIG_STA_SUPPORT */
#ifdef TP_WSC_LED_SUPPORT
	/*printk("************it should turn off wps led\n");*/
	TP_SetWSCLedStatus(TP_WSC_LED_END);
	TP_CheckWSCLEDStatus(pWscControl);
#endif
...
#endif /* APCLI_SUPPORT */
#ifdef TP_WSC_LED_SUPPORT
	TP_SetWSCLedStatus(TP_WSC_LED_START);
#endif
...
#endif /* WSC_LED_SUPPORT */
#ifdef TP_WSC_LED_SUPPORT
			TP_SetWSCLedStatus(TP_WSC_LED_PBC_OVERLAPPED);
#endif
	...
	if (CurOpMode == AP_MODE)
	{
		if ((OpMode & 0x0F) == AP_MODE)
		{
			...
			if (pWscControl->WscConfStatus == WSC_SCSTATE_UNCONFIGURED)
			{
				char ssid[33] = {0};
				UCHAR tempByte[10] = {0};
				UCHAR tempRandomByte = RandomByte(pAd);
				int index = 0;
				for (index = 0; index < 3; index++)
				{
					tempByte[index] = tempRandomByte;
					tempRandomByte = RandomByte(pAd);
				}
				NdisMoveMemory(tempByte+index, pCredential->MacAddr + 4, 2);
				sprintf(ssid, "TP-LINK_WPS%02x%02x%02x%02x%02x", tempByte[0], tempByte[1], tempByte[2], tempByte[3], tempByte[4]);
				strcpy(pCredential->SSID.Ssid, ssid);
				pCredential->SSID.SsidLength = strlen(pCredential->SSID.Ssid);
			}			
			else
			{
...
//Added by Zeng Yi. 2011-12-01. Added for WPS Callback.
extern void register_simple_config_callback (void *callback, void *arg);
extern void unregister_simple_config_callback (void);
irqreturn_t WSC_HDR_BTN_HANDLER(int irq, void *pAd, struct pt_regs *regs)
{
#ifdef CONFIG_AP_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
		{
			if(((PRTMP_ADAPTER)pAd)->net_dev->flags & IFF_UP)
			{		
				POS_COOKIE pObj = (POS_COOKIE) ((IN	PRTMP_ADAPTER)pAd)->OS_Cookie;
    			pObj->ioctl_if = 0;
                if (1 == irq)
                {
                    pObj->ioctl_if_type = INT_APCLI;
                }
				Set_AP_WscMode_Proc((IN	PRTMP_ADAPTER)pAd, (PUCHAR)"2"); /* 2: PBC */
				Set_AP_WscGetConf_Proc((IN	PRTMP_ADAPTER)pAd, (PUCHAR)"1"); /* 1: Trigger */
			}
		}
#endif // CONFIG_AP_SUPPORT //
}
EXPORT_SYMBOL(WSC_HDR_BTN_HANDLER);
//end added
	...
	IN	PRTMP_ADAPTER	pAd)
{
	...
	register_simple_config_callback(WSC_HDR_BTN_HANDLER, pAd);
