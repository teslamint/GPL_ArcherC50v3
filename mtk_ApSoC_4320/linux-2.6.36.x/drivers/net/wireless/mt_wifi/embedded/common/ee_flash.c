...
#define EEPROM_DFT_FILE_DIR	"/var/Wireless/RT2860AP/"
#ifndef EEPROM_1ST_FILE_DIR
#define EEPROM_1ST_FILE_DIR	"/var/Wireless/RT2860AP/"
#endif
...
VOID rtmp_ee_flash_write_all(PRTMP_ADAPTER pAd, USHORT *Data)
{
	...
	{
		printk("\n####  write 2.4G cal to flash !! ####\n");
		spiflash_ioctl_read(buf, DEFAULT_RF_OFFSET, EEPROM_SIZE_ALL);
		spiflash_ioctl_erase(DEFAULT_RF_OFFSET, EEPROM_SIZE);
		for (index = 0; index < EEPROM_SIZE; index++)
		{
			buf[index] = pAd->eebuf[index];
		}
		spiflash_ioctl_write(buf, DEFAULT_RF_OFFSET, EEPROM_SIZE_ALL);
	}

	if (buf)
		os_free_mem(NULL, buf);	
	//spiflash_ioctl_erase(DEFAULT_RF_OFFSET, EEPROM_SIZE);
	//spiflash_ioctl_write(pAd->eebuf, DEFAULT_RF_OFFSET, EEPROM_SIZE);	
#endif /* MULTIPLE_CARD_SUPPORT */
}
...
static NDIS_STATUS rtmp_ee_flash_init(PRTMP_ADAPTER pAd, PUCHAR start)
{
	...
	if (validFlashEepromID(pAd) == FALSE)
	{
		...
		/*no need to write back, just keep it in memory*/
