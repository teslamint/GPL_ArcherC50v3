...
COUNTRY_CODE_TO_COUNTRY_REGION allCountry[] = {
	...
	{600,	"PY",	"PARAGUAY",				TRUE,	A_BAND_REGION_4,	TRUE,	G_BAND_REGION_1},
	...
	IN RTMP_IOCTL_INPUT_STRUCT *pIoctlCmdStr)
{
	...
	INT Status = NDIS_STATUS_SUCCESS;
	RTMP_STRING *tmp = pIoctlCmdStr->u.data.pointer;
	// int len = pIoctlCmdstr->u.data.length;
	RTMP_STRING* arg = NULL;
	int len = 8192;

	os_alloc_mem(NULL, (PUCHAR *)&value, len);
	memset(value, 0, len);
	...
	while ((this_char = strsep((char **)&pIoctlCmdStr->u.data.pointer, ",")) != NULL)
	{
		...
		arg = strchr(this_char,'=');
		if (arg && strlen(arg) > 1)
		{
			*arg = '\0';
			arg++;
			NdisMoveMemory(value, arg, strlen(arg));
		}
	...
	value[len-1] = '\0';
	len = strlen(value) + 1;
	if (len > pIoctlCmdStr->u.data.length)
	{
		len = pIoctlCmdStr->u.data.length;
		value[len-1] = '\0';
	}
	// printk("func:%s(%d), copy %d bytes\n", __func__, __LINE__, len);
	pIoctlCmdStr->u.data.length = len;
	if (copy_to_user(tmp, value, len) != 0)
	{
		Status = -EFAULT;
	}

	os_free_mem(NULL, (PUCHAR)value);
	value = NULL;
...
#ifdef CONFIG_ATE
	bss.rssi = (unsigned int)(pAd->ATECtrl.LastRssi0 - pAd->BbpRssiToDbmDelta);
#else
	bss.rssi = (unsigned int)(pAd->ApCfg.RssiSample.LastRssi[0] - pAd->BbpRssiToDbmDelta);
#endif
	...
	IN RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	for (index = 0; index < MAX_LEN_OF_MAC_TABLE; index++)
	{
			...
			&& (pEntry->Sst == SST_ASSOC) && (pAd->MacTab.Content[index].func_tb_idx == apidx))
		{
			...
			/*
			printk("index %d,maxStaNum %d, max macro %d, sta mac %x:%x, vap name %s, rx %d, tx %d, assoc Num %d.\n", index,maxStaNum, maxMacro,pEntry->Addr[4], pEntry->Addr[5],
			vapName, staInfo[assocNum].rx_packets, staInfo[assocNum].tx_packets, assocNum);
			*/
			assocNum++;
		}
	}
...
#define RSSI_BASE (-127)
	...
	for(i=0; i<pAd->ScanTab.BssNr ;i++)
	{
		...
		pBssInfo[i].channel = (unsigned int)pBss->Channel;
		rssi_db = pBss->Rssi;
		/*printk("@@@pBss->Rssi %d.\n", rssi_db);*/
		rssi_dbm = rssi_db - RSSI_BASE;
		pBssInfo[i].rssi = (unsigned int)rssi_dbm;
		//printk("Bss %s auth is %d, wepStatus is %d\n", pBss->Ssid, pBss->AuthMode, pBss->WepStatus);
		if (FALSE == pBss->Privacy)
		{
			pBssInfo[i].securityEnable = FALSE;
		}
		else
		{
			pBssInfo[i].securityEnable = TRUE;
		}

		pBssInfo[i].authMode = pBss->AuthMode;
		pBssInfo[i].wepStatus = pBss->WepStatus;

		memcpy(pBssInfo[i].ssid, pBss->Ssid, MAX_LEN_OF_SSID);
		memcpy(pBssInfo[i].bssid, pBss->Bssid, MAC_ADDR_LEN);
	}

	if(maxBssNum > pAd->ScanTab.BssNr) {
		maxBssNum = pAd->ScanTab.BssNr;
	}
	wrq->u.data.length = sizeof(CHAR)*(maxBssNum * sizeof(WLAN_BSS_INFO)) + sizeof(unsigned int);
	copy_to_user(wrq->u.data.pointer, &maxBssNum, sizeof(maxBssNum));
	copy_to_user(wrq->u.data.pointer + sizeof(maxBssNum), pBssInfo, (maxBssNum * sizeof(WLAN_BSS_INFO)));
	kfree(pBssInfo);
}
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	*wrq)
{
	WLAN_RATE_INFO rate;
	POS_COOKIE	pObj = pAd->OS_Cookie;
	INT rateMask[] = {2, 4, 11, 22, 12, 18, 24, 36, 48, 72, 96, 108};
	INT supRateMask[] = {0x82, 0x84, 0x8B, 0x96, 0, 0x12, 0, 0x24, 0, 0x48, 0, 0x6c};
	INT rateMaskLen = sizeof(rateMask) / sizeof(INT);
	INT supRateMaskLen = sizeof(supRateMask) / sizeof(INT);
	INT apidx = pObj->ioctl_if;
	...
	wrq_t = (struct iwreq *)wrq;
	rate.basicRateMask = pAd->CommonCfg.BasicRateBitmap;
	rate.basicMcsMask = 0;

	for (i = 0; i < supRateMaskLen; i++)
	{
		if (supRateMask[i] == 0)
			continue;
		if (pAd->CommonCfg.SupRate[j++] == supRateMask[i])
		{
	...
	wrq_t->u.data.length = sizeof(rate);
	copy_to_user(wrq->u.data.pointer, &rate, wrq_t->u.data.length);
}
	...
	IN RTMP_STRING *arg)
{
	...
	else
	{
		...
		*/
		/* REVIEW: Not sure about this one. Should we just use ApSiteSurvey? */
#ifdef APCLI_AUTO_CONNECT_IMPROVE_SUPPORT
		if (pApCfg->ApCliAutoConnectRunning == TRUE)
		{
	...
	RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	printk("Write Caldata to flash\n");

	rtmp_ee_write_flash(pAd, arg);
