...
#ifdef TP_DEBUG_CMD
INT Show_Channel_Info_Proc(RTMP_ADAPTER *pAd, PSTRING arg)
{
	INT i;
	for (i=0;i<pAd->ChannelListNum;i++)
	{
		printk("BuildChannel # %d :: Pwr0 = %d, Pwr1 =%d, Flags = %x\n ", 
...
INT Show_Repeater_Cli_Table_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
    //INT i;
    //UINT32 RegValue;
    //ULONG DataRate=0;
    INT apIdx = 0, cliIdx = 0;
    ...
    printk("RptCliSize: %d\n", pAd->ApCfg.RepeaterCliSize);
    printk("\n%-4s%-19s%-19s%-4s%-6s%-4s%-9s%-8s%-5s%-10s%-10s%-10s%-10s%-11s%-11s%-12s\n",
           "Idx", "OriMac", "RptMac", "En", "Valid", "Eth", "ApCliIdx",
           "LinkIdx", "WCID", "ConnState", "CtrlState",
           "SyncState", "AuthState", "AssocState", "AuthReqCnt", "AssocReqCnt");

    for (cliIdx = 0; cliIdx< MAX_EXT_MAC_ADDR_SIZE; cliIdx++)
    {
        PREPEATER_CLIENT_ENTRY pRptCliEntry = &pAd->ApCfg.ApCliTab[apIdx].RepeaterCli[cliIdx];


        printk("%-4d", cliIdx);
        printk("%02X:%02X:%02X:%02X:%02X:%02X  ",
               pRptCliEntry->OriginalAddress[0], pRptCliEntry->OriginalAddress[1],
               pRptCliEntry->OriginalAddress[2], pRptCliEntry->OriginalAddress[3],
               pRptCliEntry->OriginalAddress[4], pRptCliEntry->OriginalAddress[5]);

        printk("%02X:%02X:%02X:%02X:%02X:%02X  ",
               pRptCliEntry->CurrentAddress[0], pRptCliEntry->CurrentAddress[1], pRptCliEntry->CurrentAddress[2],
               pRptCliEntry->CurrentAddress[3], pRptCliEntry->CurrentAddress[4], pRptCliEntry->CurrentAddress[5]);
        printk("%-4d", (int)pRptCliEntry->CliEnable);
        printk("%-4d", (int)pRptCliEntry->CliValid);

        printk("%-4d", (int)pRptCliEntry->bEthCli);
        printk("%-4d", (int)pRptCliEntry->MatchApCliIdx);
        printk("%-4d", (int)pRptCliEntry->MatchLinkIdx);
        printk("%-4d", (int)pRptCliEntry->MacTabWCID);
        printk("%-4d", (int)pRptCliEntry->CliConnectState);
        printk("%-4d", (int)pRptCliEntry->CtrlCurrState);
        printk("%-4d", (int)pRptCliEntry->SyncCurrState);
        printk("%-4d", (int)pRptCliEntry->AuthCurrState);
        printk("%-4d", (int)pRptCliEntry->AssocCurrState);
        printk("%-4d", (int)pRptCliEntry->AuthReqCnt);
        printk("%-4d", (int)pRptCliEntry->AssocReqCnt);
        printk("\n");
    }
