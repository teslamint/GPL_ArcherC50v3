...
#if 1 /* limit the packets already in all tr_entry txq */
    sta_num=0;
...
INT ap_rx_pkt_allow(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
	...
	{
		PREPEATER_CLIENT_ENTRY pRptCliEntry = NULL;
		UCHAR isLinkValid = 0;

		pRptCliEntry = RTMPLookupRepeaterCliEntry(pAd, FALSE, pHeader->Addr3, TRUE, &isLinkValid);

		if (pRptCliEntry && (pRptCliEntry->CliValid))
		{
			return false;
		}
	}
