...
VOID ApCliIfMonitor(RTMP_ADAPTER *pAd)
{
	...
	for(index = 0; index < MAX_APCLI_NUM; index++)
	{
		...
		if ((pAd->ApCfg.bMACRepeaterEn) && (pApCliEntry->Enable))
		{
			for(CliIdx = 0; CliIdx < MAX_EXT_MAC_ADDR_SIZE; CliIdx++)
			{
				...
				if (pReptCliEntry->CliEnable)
				{
					...
					if (!VALID_WCID(Wcid))
						{
							if (RTMP_TIME_AFTER(pAd->Mlme.Now32, (pReptCliEntry->CliTriggerTime + (8 * OS_HZ))))
							{
								MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
										 ("RptCliEntry timeout -- %02x:%02x:%02x:%02x:%02x:%02x\n!!!",
										  PRINT_MAC(pReptCliEntry->OriginalAddress)));
					...
					else
					{
						...
						{
							MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
									 ("%s(): Remove invalid repeater entry: ApCli%d, CliIdx = %d\n",
		...
		if (pApCliEntry->Valid == TRUE)
		{
            ...
            {
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
                         ("(%s)Beacon Recive Time Out, CurTime: %lu, ApCliRcvBeaconTime: %lu\n", 
                          __FUNCTION__, pAd->Mlme.Now32, pApCliEntry->ApCliRcvBeaconTime));
				bForceBrocken = TRUE;
            }
		...
		if (apcli_entry && (&apcli_entry->wdev == wdev))
		{
			...
			if (pAd->ApCfg.bMACRepeaterEn == TRUE)
			{
				...
				else
				{
					...
					{
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("==> ApCliAllowToSendPacket():Receive trigger packet!!! pWscControl->WscConfMode = %d, pWscControl->bWscTrigger == %d!!!\n",
							pWscControl->WscConfMode, pWscControl->bWscTrigger));
						return FALSE;
					}
#endif /*WSC_AP_SUPPORT*/
	...
	IN  PRTMP_ADAPTER   pAd)
{
	...
	*/
			...
			pSsidBssTab->BssNr <=MAX_LEN_OF_BSS_TABLE)
	{
		...
		*/
		...
		if (pAd->CommonCfg.Channel != pBssEntry->Channel ||
			(pBssEntry->AddHtInfo.AddHtInfo.RecomWidth == HT_BW_40 &&
				(pBssEntry->AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_ABOVE || pBssEntry->AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_BELOW) &&
				pAd->CommonCfg.RegTransmitSetting.field.EXTCHA != pBssEntry->AddHtInfo.AddHtInfo.ExtChanOffset) ||
			pBssEntry->AddHtInfo.AddHtInfo.RecomWidth != pAd->CommonCfg.RegTransmitSetting.field.BW)
		{
			...
			/* REVIEW: Not sure about this one. We might fix the same thing as MTK did. */
			...
			if (pBssEntry->AddHtInfo.AddHtInfo.RecomWidth == HT_BW_40)
			{
				/* DUT EXTCHA follows RootAp when BW=40M. By Deng Zhong 2016-03-14*/
				if(pBssEntry->AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_ABOVE || pBssEntry->AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_BELOW)
				{
					/*  EXTCHA_ABOVE or EXTCHA_BELOW follow, EXTCHA_NONE can not follow */
			pAd->CommonCfg.RegTransmitSetting.field.EXTCHA = pBssEntry->AddHtInfo.AddHtInfo.ExtChanOffset;
				}
				Set_HtBw_Proc(pAd, "1");
			}
			...
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("(%s-%d)ChannelWidth: %d, Bw: %d, Extcha: %d\n", 
	...
	else
	{
        ...
        pAd->ApCfg.ApCliTab[0].Enable = TRUE;
#endif
        //MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("I/F(apcli%d) Set_ApCli_Enable_Proc::(enable = %d)\n", 0, pAd->ApCfg.ApCliTab[0].Enable));

#ifdef APCLI_CONNECTION_TRIAL
        if (pAd->ApCfg.ApCliTab[0].TrialCh == 0)
#endif
            ApCliIfDown(pAd);
		pAd->ApCfg.ApCliAutoConnectRunning = FALSE;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<--- ApCliAutoConnectExec()\n"));
