...
extern INT Set_ApCli_AutoConnect_Proc(
	...
	IN MLME_QUEUE_ELEM *Elem)
{
	...
	if (pApCliEntry->ProbeReqCnt == 7)
	{
		...
		Set_ApCli_AutoConnect_Proc(pAd, "2");	//restart partial scan RootAp 
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("IOCTLType: %x\n",  ((POS_COOKIE)pAd->OS_Cookie)->ioctl_if_type));
		pObj->ioctl_if_type = old_if_type;
		
		*pCurrState = APCLI_CTRL_DISCONNECTED;
		/* reset all the timers */
		//RTMPCancelTimer(&(pApCliEntry->MlmeAux.ProbeTimer), &Cancelled);
		//RTMPSetTimer(&(pApCliEntry->MlmeAux.ProbeTimer), (pAd->ChannelListNum) * MIN_CHANNEL_TIME);
		return;
	}
