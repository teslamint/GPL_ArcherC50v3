	...
	IN MLME_QUEUE_ELEM *Elem) 
{
								...
								TRUE))
	{
		...
		*/
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("ApCliPeerProbeRspAtJoinAction MsgChannel=%d, ie_list->Channel=%d, pAd->CommonCfg.Channel=%d\n", Elem->Channel, ie_list->Channel, pAd->CommonCfg.Channel));
		if((pAd->CommonCfg.Channel != ie_list->Channel) && (pAd->ApCfg.ApCliTab[ifIndex].Enable == TRUE))
		{
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerBeaconAndProbeRspSanity Elem->Channel=%d, ie_list->Channel=%d, pAd->CommonCfg.Channel=%d\n", Elem->Channel, ie_list->Channel, pAd->CommonCfg.Channel));
			goto LabelErr;
		}
		...
		if (matchFlag)
		{
...
#ifdef TP_PRIVATE_REQUIRE
#ifdef DOT11N_DRAFT3
			/* close the BssCoex at repeater mode. added by Deng Zhong 2016-03-17 */
				...
				WMODE_CAP_N(PhyMode))
			{
				...
				if (ie_list->AddHtInfoLen > 0)
				{
					...
					MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PeerBeaconAtJoinAction HT===>CentralCh = %d, ControlCh = %d, RecomWidth = %d\n",
									CentralChannel, ie_list->AddHtInfo.ControlChan,
									ie_list->AddHtInfo.AddHtInfo.RecomWidth));

					if ((ie_list->AddHtInfo.AddHtInfo.RecomWidth != pAd->CommonCfg.RegTransmitSetting.field.BW) ||
						((ie_list->AddHtInfo.AddHtInfo.RecomWidth == HT_BW_40) &&
						(pAd->CommonCfg.RegTransmitSetting.field.EXTCHA != ie_list->AddHtInfo.AddHtInfo.ExtChanOffset) &&
						(ie_list->AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_ABOVE || ie_list->AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_BELOW)))
					{
						UCHAR tempBuf[20];
						if (ie_list->AddHtInfo.AddHtInfo.RecomWidth == HT_BW_40)
						{
							/* DUT EXTCHA follows RootAp when BW=40M. By Deng Zhong 2016-03-14*/
							if((ie_list->AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_ABOVE) || (ie_list->AddHtInfo.AddHtInfo.ExtChanOffset == EXTCHA_BELOW))
						...
						sprintf(tempBuf, "%d", pApCliEntry->MlmeAux.Channel);
						MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("(%s-%d)Switch to channel :%s BW:%d EXTCHA:%d\n",
