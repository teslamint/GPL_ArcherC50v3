...
BOOLEAN MacTableDeleteEntry(RTMP_ADAPTER *pAd, USHORT wcid, UCHAR *pAddr)
{
	...
	if (pEntry && !IS_ENTRY_NONE(pEntry))
	{
		...
		if (MAC_ADDR_EQUAL(pEntry->Addr, pAddr))
		{
            ...
            if (likely (pAd->MacTab.Size > 0))
            {
			pAd->MacTab.Size--;
            }
            else
            {
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Impossible remove, a insert action must be happened during mac_table_reset\n"));
            }
