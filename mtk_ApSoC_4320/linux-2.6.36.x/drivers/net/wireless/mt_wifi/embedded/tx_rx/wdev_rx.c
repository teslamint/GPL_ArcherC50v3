...
MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s():  No DATA!\n", __FUNCTION__));
    ...
    if (pEntry && IS_ENTRY_APCLI(pEntry)) {
        ...
        /* Update ApCliRcvbeacontime here to avoid beacon missing. */
        if (pEntry->func_tb_idx < MAX_APCLI_NUM)
        {
            ...
            pAd->ApCfg.ApCliTab[ApCliIndex].ApCliRcvBeaconTime = Now32;
        }
				goto ret;
    }
	else
...
#ifdef MAC_REPEATER_SUPPORT
            if (pHeader->FC.FrDs == 1 && pHeader->FC.ToDs == 0 && RTMPQueryLookupRepeaterCliEntryMT(pAd, pHeader->Addr3))
            {
                MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): drop apcli0 loopback packet!\n", __FUNCTION__));
                RELEASE_NDIS_PACKET(pAd, pRxPacket, NDIS_STATUS_SUCCESS);
                continue;
            }
#endif
