...
#if defined (CONFIG_RAETH_EXTERNAL_QCA8337)
...
spiflash_ioctl_read(addr.sa_data, TP_MAC_OFFSET, 6);
printk("Read MAC from flash(%8x) %02x-%02x-%02x-%02x-%02x-%02x\n", TP_MAC_OFFSET,
...
#ifdef CONFIG_RAETH_NAPI
#if LINUX_VERSION_CODE > KERNEL_VERSION(2,6,35)
	netif_napi_add(dev, &ei_local->napi, raeth_clean, 8);
#endif
#endif
...
void rt_gsw_init(void)
{
	...
	*(unsigned long *)(RALINK_ETH_SW_BASE+0x3600) = 0x5f33b;//CPU Port6 Force Link 1G, FC ON
	*(unsigned long *)(RALINK_ETH_SW_BASE+0x0010) = 0x7f7f7fe0;//Set Port6 CPU Port
        ...
        *(unsigned long *)(RALINK_ETH_SW_BASE+0x3500) = 0x5f33b;//(P5, Force mode, Link Up, 1000Mbps, Full-Duplex, FC ON)
...
/* set MT7530 central align */
	...
	*(unsigned long *)(RALINK_ETH_SW_BASE+0x3400) = 0x5f33b;//(P4, Force mode, Link Up, 1000Mbps, Full-Duplex, FC ON)
	*(unsigned long *)(0xb0000060) &= ~(1 << 10); //set GE2 to Normal mode
	//rxclk_skew, txclk_skew = 0
	*(unsigned long *)(SYSCFG1) &= ~(0x3 << 14); //GE2_MODE=RGMii Mode
...
#if defined(CONFIG_TP_MODEL_WR840NV4) || defined(CONFIG_TP_MODEL_C50V3) || \
        defined(CONFIG_TP_MODEL_WR841HPV5) || defined(CONFIG_TP_MODEL_C20V4) ||\
        defined(CONFIG_TP_MODEL_WR802NV4)
